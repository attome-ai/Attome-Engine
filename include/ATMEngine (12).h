...
// Rendering systems for SDL_GPU
//------------------------------

// Material system for GPU rendering
struct GPUMaterial {
    ...
    int sampler_id;            // Sampler ID for textures
...
// Mesh data for GPU rendering with LOD support
struct GPUMesh {
    int vertex_buffer_id;     // Vertex buffer ID
    int index_buffer_id;      // Index buffer ID
...
// Render batch for GPU-driven rendering
struct ALIGN_TO_CACHE GPURenderBatch {
    int material_id;           // Material ID
    int shader_id;             // Shader ID
    int mesh_id;               // Mesh ID
    int instance_buffer_id;    // Instance buffer ID
...
// GPU Resource Management using SDL3_GPU
//----------------------

// GPU resources using SDL3 GPU API
struct GPUResources {
    ...
    struct Shader {
        ...
        std::unordered_map<std::string, int> bindings;  // Texture/buffer bindings
    };
    ...
    // Transfer buffers for uploading data
    SDL_GPUTransferBuffer** transfer_buffers;
    int transfer_buffer_count;
    int transfer_buffer_capacity;

    GPUResources(int max_textures, int max_shaders, int max_buffers, int max_meshes, int max_materials, int max_render_targets, int max_samplers, int max_transfer_buffers);
    ~GPUResources();
    ...
    int createVertexBuffer(const void* data, size_t size, SDL_GPUBufferUsageFlags usage);
    int createIndexBuffer(const void* data, size_t size, SDL_GPUBufferUsageFlags usage);
    int createUniformBuffer(const void* data, size_t size);
    int createStorageBuffer(const void* data, size_t size, bool writable);
    int createTransferBuffer(size_t size);
    int createSampler(SDL_GPUFilter min_filter, SDL_GPUFilter mag_filter, SDL_GPUSamplerAddressMode address_mode);
    int createMesh(const float* vertices, int vertex_count, const int* indices, int index_count);
    int createMaterial(int diffuse_texture, int normal_texture, int specular_texture, int sampler_id, const glm::vec4& diffuse_color, const glm::vec4& specular_color, float shininess);
    int createShader(const void* vertex_code, size_t vertex_code_size, const void* fragment_code, size_t fragment_code_size, SDL_GPUShaderFormat format);
    int createRenderTarget(int width, int height, SDL_GPUTextureFormat format, SDL_GPUTextureUsageFlags usage);

    // Resource updates
    void updateBuffer(int buffer_id, const void* data, size_t size, size_t offset = 0);
    void updateTexture(int texture_id, const void* data, int width, int height, int x = 0, int y = 0);

    // Resource destruction
    void destroyTexture(int texture_id);
    void destroyBuffer(int buffer_id);
    void destroyShader(int shader_id);
    void destroyMesh(int mesh_id);

    // Helper methods
    SDL_GPUBuffer* getVertexBuffer(int id) const;
    SDL_GPUBuffer* getIndexBuffer(int id) const;
    SDL_GPUBuffer* getUniformBuffer(int id) const;
    SDL_GPUTexture* getTexture(int id) const;
    SDL_GPUGraphicsPipeline* getPipeline(int shader_id) const;
};

// GPU Renderer for SDL3 GPU API
class GPURenderer {
private:
    // Command buffer handling
    SDL_GPUCommandBuffer* current_command_buffer;
    std::vector<SDL_GPUCommandBuffer*> command_buffers;
    ...
    SDL_GPUFence* last_frame_fence;
    int frames_in_flight;

    // Swapchain state
    SDL_GPUTexture* swapchain_texture;

    // Default pipelines & resources
...
public:
    GPURenderer(SDL_GPUDevice* device, SDL_Window* window, int max_frames_in_flight = 2);
    ~GPURenderer();
    ...
    void beginRenderPass(SDL_GPUTexture* color_target, SDL_GPUTexture* depth_target = nullptr);
    void beginSwapchainRenderPass();
    void endRenderPass();

    // Draw operations
    void drawMesh(int mesh_id, int material_id, int shader_id, const glm::mat4& transform);
    void drawMeshInstanced(int mesh_id, int material_id, int shader_id, int instance_buffer_id, int instance_count);
    void drawBatch(const GPURenderBatch& batch, const EntityManager& entities);

    // Resource binding
    void setShader(int shader_id);
    void setVertexBuffer(int buffer_id, uint32_t binding = 0);
    void setIndexBuffer(int buffer_id);
    void setTexture(int texture_id, uint32_t binding);
    void setSampler(int sampler_id, uint32_t binding);
    void setUniformBuffer(int buffer_id, uint32_t binding);

    // State setting
    void setViewport(int x, int y, int width, int height);
    void setScissor(int x, int y, int width, int height);
    void pushUniformData(const void* data, size_t size, SDL_GPUShaderStage stage, int slot);
...
private:
    // Internal helpers
    SDL_GPUCommandBuffer* getCurrentCommandBuffer();
