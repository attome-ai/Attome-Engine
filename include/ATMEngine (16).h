...
// Modify EntityChunk structure to include type_id
// In the header file (ATMEngine.h)
...
// Quadtree node structure for efficient spatial partitioning
typedef struct QuadTreeNode {
    SDL_FRect boundary;       // Boundary of this node
    int* entities;            // Array of entity indices in this node
    int entity_count;         // Number of entities in this node
    int entity_capacity;      // Capacity of the entities array
    bool is_divided;          // Whether this node is divided

    // Child nodes (null if not divided)
...
// QuadTree for efficient entity queries
typedef struct QuadTree {
    QuadTreeNode* root;       // Root node of the quadtree
    int max_entities;         // Maximum entities per node before subdivision
    int max_depth;            // Maximum depth of the tree
    float world_width;        // Width of the world space
    float world_height;       // Height of the world space
} QuadTree;
...
typedef struct Engine {
    ...
    QuadTree quadtree;          // Changed from SpatialGrid to QuadTree
    RenderBatch* batches;
    int batch_count;
    TextureAtlas atlas;
    Camera camera;
...
void quadtree_query(QuadTree* quadtree, SDL_FRect query_rect,
