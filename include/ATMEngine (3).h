...
typedef struct EntityManager {
    ...
    // Child entity system (cache-aligned arrays)
    int* parent_id;        // parent entity ID (-1 for root entities) - aligned
    int* first_child_id;   // ID of first child (-1 if none) - aligned
    int* next_sibling_id;  // ID of next sibling (-1 if none) - aligned
    int* child_count;      // number of children - aligned
    float* local_x;        // local x position relative to parent - aligned
    float* local_y;        // local y position relative to parent - aligned
...
Engine* engine_create(int window_width, int window_height, int world_width, int world_height, int cell_size = 128 * 4);
...
// Child entity management functions
