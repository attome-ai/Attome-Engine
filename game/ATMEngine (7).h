...
static constexpr uint32_t GRID_CELL_SIZE = 1024;
...
private:
    ...
    alignas(64) Task* taskQueue[MAX_TASKS];
    ...
    alignas(64) std::atomic<bool> hasWork{ false };
    std::condition_variable workAvailable;
    std::mutex workMutex;

    // Function to set thread priority and affinity
    void setThreadPriorityAndAffinity(uint32_t threadIndex, uint32_t threadCount);

    void workerThread(uint32_t threadIndex, uint32_t threadCount) {
        // Set priority and affinity at the start of the thread
        setThreadPriorityAndAffinity(threadIndex, threadCount);
        ...
        while (running) {
            ...
            while (running) {
                ...
                if (tail.compare_exchange_weak(current, current + 1,
                    std::memory_order_relaxed)) {
                    task = taskQueue[current % MAX_TASKS];
                    break;
                }
            }
            ...
            if (task) {
                task->func();
                delete task;
                activeTaskCount.fetch_sub(1, std::memory_order_release);
            }
        }
    }

public:
    void init(uint32_t threadCount = std::thread::hardware_concurrency() - 4) {
        for (uint32_t i = 0; i < threadCount; ++i) {
            // Create threads with thread index and count parameters
            threads.emplace_back(&TaskSystem::workerThread, this, i, threadCount);
        }
    }
    ...
    template<typename F>
    void submit(F&& func) {
        while (head - tail >= MAX_TASKS) {
            // Queue is full, spin
            std::this_thread::yield();
        }

        taskQueue[head % MAX_TASKS] = new Task(std::forward<F>(func));
        activeTaskCount.fetch_add(1, std::memory_order_release);
    ...
    void waitForCompletion() {
        while (activeTaskCount.load(std::memory_order_acquire) > 0) {
            std::this_thread::yield();
        }
    }
};
...
private:
    struct Cell {
        static constexpr size_t MAX_ENTITIES_PER_CELL = 571;  // Fixed capacity
...
public:
    ...
    void add2(const EntityRef& entity, const uint16_t x, const uint16_t y) {
        ...
        if (index < Cell::MAX_ENTITIES_PER_CELL) {
