...
static constexpr int32_t WORLD_WIDTH = 1000 * POSITION_UNIT;
static constexpr int32_t WORLD_HEIGHT = 1000 * POSITION_UNIT;
static constexpr int32_t GRID_CELL_SIZE = 256 * POSITION_UNIT;
...
public:
    ...
    void printQueryCircle(int32_t centerX, int32_t centerY, float radius);
    void printQueryRect(int32_t minX, int32_t minY, int32_t maxX, int32_t maxY);
};
...
public:
    ...
    uint64_t last_frame_time;  // Changed from Uint64 to uint64_t
    float fps;
    ...
    void updateEntityTypes(uint32_t delta_time);  // Changed from float to uint32_t
    void renderScene();
...
// Helper function to get texture region
