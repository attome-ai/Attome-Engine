...
struct VulkanTexture {
    ...
    uint32_t width;
    uint32_t height;
    SDL_FRect region;  // UV region for atlas textures
};

// Vulkan batch representation
struct VulkanBatch {
    int textureId;
    int zIndex;
    std::vector<VulkanVertex> vertices;
    std::vector<uint32_t> indices;
    // Device-local buffers (GPU optimized)
    VkBuffer vertexBuffer;
    VkDeviceMemory vertexMemory;
    VkBuffer indexBuffer;
    VkDeviceMemory indexMemory;
    // Staging buffers (CPU accessible)
    ...
    VulkanBatch(int textureId, int zIndex, int initialVertexCapacity = VulkanConfig::INITIAL_BATCH_VERTEX_CAPACITY)
        ...
        vertexStagingBuffer(VK_NULL_HANDLE), vertexStagingMemory(VK_NULL_HANDLE),
        indexStagingBuffer(VK_NULL_HANDLE), indexStagingMemory(VK_NULL_HANDLE),
        fence(VK_NULL_HANDLE) {
        vertices.reserve(initialVertexCapacity);
        indices.reserve(initialVertexCapacity * 1.5f);
    }
...
private:
    ...
    std::vector<VkFence> imagesInFlight;  // Track which fences are associated with which swapchain images
