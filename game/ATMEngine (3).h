...
static constexpr uint32_t WORLD_WIDTH =  10000;
static constexpr uint32_t WORLD_HEIGHT = 10000;
...
private:
    struct Cell {
        static constexpr size_t INITIAL_CAPACITY = 500;  // Adjust based on your needs
...
public:
    ...
    void add(const EntityRef& entity, const float& x, const float& y) {
        const float& outCellX = x * INV_GRID_CELL_SIZE;
        const float& outCellY = y * INV_GRID_CELL_SIZE;

        cells[outCellY][outCellX].entities.push_back(entity);
    }
    void add2(const EntityRef& entity, const uint16_t& x, const uint16_t  y) 
    {
        cells[y][x].entities.push_back(entity);
    ...
    const std::vector<EntityRef>& queryCircle(float centerX, float centerY, float radius) {
        ...
        for ( uint16_t cy = minCellY; cy <= maxCellY; ++cy) {
            for (uint16_t cx = minCellX; cx <= maxCellX; ++cx) {
                ...
                for (const auto& entity : cells[cy][cx].entities) {
    ...
    // Assuming Engine has a way to get entity positions and a way to iterate through all entities
