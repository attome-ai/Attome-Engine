...
void RenderBatch::addQuadRotated(float x, float y, float w, float h,
                                 float radians, SDL_FRect tex_region) {
  ...
  const float u[4] = {tex_region.x, tex_region.x + tex_region.w,
                      tex_region.x + tex_region.w, tex_region.x};
  const float t[4] = {tex_region.y, tex_region.y, tex_region.y + tex_region.h,
                      tex_region.y + tex_region.h};
  for (int i = 0; i < 4; ++i) {
    v[i].position = {cx + corner_x[i] * c - corner_y[i] * sn,
                     cy + corner_x[i] * sn + corner_y[i] * c};
    v[i].color = {1, 1, 1, 1};
    v[i].tex_coord = {u[i], t[i]};
  }
...
void RenderBatchManager::addQuadRotated(int textureId, int zIndex, float x,
                                        float y, float w, float h,
                                        float radians, SDL_FRect tex_region) {
  RenderBatch &batch = getBatch(textureId, zIndex);
  batch.addQuadRotated(x, y, w, h, radians, tex_region);
}
...
void update_cell_coords(const SpatialGrid &grid, EntityContainer *container,
                        uint32_t slot, float x, float y) {
  if (!container || slot >= static_cast<uint32_t>(container->count)) {
    return;
  }

  int cell_x = static_cast<int>(x * grid.invCellSize());
  int cell_y = static_cast<int>(y * grid.invCellSize());
  cell_x = std::clamp(cell_x, 0, static_cast<int>(grid.cellsWide()) - 1);
  cell_y = std::clamp(cell_y, 0, static_cast<int>(grid.cellsHigh()) - 1);
...
bool is_rotatable(const EntityContainer *container) {
  return (container->containerFlag &
          static_cast<uint8_t>(ContainerFlag::ROTATABLE)) != 0;
}
...
bool intersects_rect(const RenderableEntityContainer *container, uint32_t slot,
  ...
  float entity_x = container->x_positions[slot];
  float entity_y = container->y_positions[slot];
  float entity_w = static_cast<float>(container->widths[slot]);
  float entity_h = static_cast<float>(container->heights[slot]);

  if (is_rotatable(container) && container->rotations[slot] != 0.0f) {
    // A rotated sprite stays inside the circle through its corners, so grow
    // the box to that circle's bounds to avoid culling visible corners.
    const float grow_x = (std::sqrt(entity_w * entity_w + entity_h * entity_h) -
                          entity_w) * 0.5f;
    const float grow_y = (std::sqrt(entity_w * entity_w + entity_h * entity_h) -
                          entity_h) * 0.5f;
    entity_x -= grow_x;
    entity_y -= grow_y;
    entity_w += grow_x * 2.0f;
    entity_h += grow_y * 2.0f;
  }
...
void add_world_quad(RenderBatchManager &batch_manager, const TextureAtlas &atlas,
  ...
  if (is_rotatable(container) && container->rotations[slot] != 0.0f) {
    batch_manager.addQuadRotated(texture_id, z_index,
                                 container->x_positions[slot],
                                 container->y_positions[slot],
                                 static_cast<float>(container->widths[slot]),
                                 static_cast<float>(container->heights[slot]),
                                 container->rotations[slot], region);
    return;
  }
...
void add_screen_quad(RenderBatchManager &batch_manager, const TextureAtlas &atlas,
  ...
  if (is_rotatable(container) && container->rotations[slot] != 0.0f) {
    batch_manager.addQuadRotated(texture_id, z_index, (world_x - cam_x1) * zoom,
                                 (world_y - cam_y1) * zoom, width * zoom,
                                 height * zoom, container->rotations[slot],
                                 region);
    return;
  }
...
int32_t StaticChunkCache::worldToChunk(float world_value) const {
  return static_cast<int32_t>(std::floor(world_value / chunk_size_));
}
...
void run_entity_updates(Engine *engine, float delta_time) {
  ...
  const float margin = engine->config.hybrid_activation_margin;
  engine->entityManager.updateHybrid(engine, delta_time, x1 - margin,
                                     y1 - margin, x2 + margin, y2 + margin);
...
void engine_update(Engine *engine) {
  ...
  delta_time = std::clamp(delta_time, 0.0f, engine->config.max_frame_dt);

  engine->last_frame_time = now;
  engine->fps = delta_time > 0.0f ? (1.0f / delta_time) : 0.0f;
  engine->frame_dt = delta_time;

  process_pending_removals(engine);

  if (engine->config.fixed_timestep_hz <= 0.0f) {
    engine->sim_dt = delta_time;
    engine->sim_steps = 1;
    run_entity_updates(engine, delta_time);
    return;
  }

  // Fixed timestep: run whole steps, carry the remainder to the next frame.
  const float step = 1.0f / engine->config.fixed_timestep_hz;
  engine->fixed_accumulator += delta_time;
  int steps = 0;
  while (engine->fixed_accumulator >= step &&
         steps < engine->config.max_fixed_steps_per_frame) {
    run_entity_updates(engine, step);
    engine->fixed_accumulator -= step;
    ++steps;
  }
  if (steps == engine->config.max_fixed_steps_per_frame &&
      engine->fixed_accumulator >= step) {
    // Too far behind (slow device / breakpoint): drop the backlog instead of
    // spiralling into ever longer frames.
    engine->fixed_accumulator = std::fmod(engine->fixed_accumulator, step);
  }
  engine->sim_dt = step;
  engine->sim_steps = steps;
  engine->fixed_alpha = engine->fixed_accumulator / step;
}
...
EntityHandle engine_create_entity(Engine *engine, int type_id) {
  ...
  update_cell_coords(engine->grid, container, slot,
                     container->x_positions[slot], container->y_positions[slot]);
...
bool engine_set_entity_position(Engine *engine, EntityHandle entity_id,
  ...
  update_cell_coords(engine->grid, container, slot, x, y);
...
bool engine_set_entity_rotation(Engine *engine, EntityHandle entity_id,
  ...
  auto *renderable =
      as_renderable(engine->entityManager.containers[type_id].get());
  if (!renderable) {
    return false;
  }
  ...
  renderable->rotations[slot] = radians;
  if (engine->entityManager.getRuntimeKind(type_id) ==
      ObjectRuntimeKind::Static) {
    engine->staticChunkCache.markNeedsFullRebuild();
  }
...
TextureAtlas::TextureAtlas(TextureAtlas &&other) noexcept
    : textures(other.textures), texture_count(other.texture_count),
      ...
      renderer(other.renderer), free_ids(std::move(other.free_ids)) {
...
int TextureAtlas::registerTexture(SDL_Surface *surface, int x, int y, int width,
  ...
  // Reuse a slot freed by unregisterTexture() before growing.
  int reuse_id = -1;
  if (!free_ids.empty()) {
    reuse_id = free_ids.back();
    free_ids.pop_back();
  } else {
  ...
  const int texture_id = reuse_id >= 0 ? reuse_id : texture_count;
  textures[texture_id] = texture;
  regions[texture_id] = {static_cast<float>(clamped_x) / surface_width,
...
void TextureAtlas::unregisterTexture(int textureId) {
  ...
  regions[textureId] = SDL_FRect{0.0f, 0.0f, 1.0f, 1.0f};
  free_ids.push_back(textureId);
}
...
Engine *create_engine(const EngineConfig &config, bool legacy_grid) {
  PROFILE_FUNCTION();
  ...
  const SDL_WindowFlags window_flags =
      config.window_resizable ? SDL_WINDOW_RESIZABLE : 0;
  if (!SDL_CreateWindowAndRenderer(config.window_title.c_str(),
...
#if !ATM_RUNTIME_GRID
  if (!legacy_grid &&
      (config.world_width != static_cast<int>(WORLD_WIDTH) ||
       config.world_height != static_cast<int>(WORLD_HEIGHT) ||
       config.grid_cell_size != static_cast<int>(GRID_CELL_SIZE))) {
    SDL_Log("[engine] grid uses the compile-time %ux%u / %upx layout; "
            "configure with -DATTOME_RUNTIME_GRID=ON to size it from config",
            WORLD_WIDTH, WORLD_HEIGHT, GRID_CELL_SIZE);
  }
#endif

  TextureAtlas atlas(renderer, config.world_width, config.world_height);
  Camera camera = {config.window_width * 0.5f, config.window_height * 0.5f,
                   static_cast<float>(config.window_width),
                   static_cast<float>(config.window_height), 1.0f};

  Engine *engine = new Engine{
      window,
      renderer,
      RenderBatchManager(),
      legacy_grid ? SpatialGrid()
                  : SpatialGrid(config.world_width, config.world_height,
                                config.grid_cell_size, config.grid_node_reserve,
                                config.grid_query_pad_cells),
      std::move(atlas),
      camera,
      SDL_FRect{0.0f, 0.0f, static_cast<float>(config.world_width),
                static_cast<float>(config.world_height)},
      SDL_GetPerformanceCounter(),
      0.0f,
      {},
      StaticChunkCache(config.static_chunk_size),
      EntityManager(),
      config};
...
Engine *engine_create(int window_width, int window_height, int world_width,
  ...
  (void)cell_size; // the legacy grid always used GRID_CELL_SIZE
  EngineConfig config;
  ...
  config.grid_cell_size = static_cast<int>(GRID_CELL_SIZE);
  return create_engine(config, /*legacy_grid=*/true);
}

Engine *engine_create_with_config(const EngineConfig &config) {
  return create_engine(config, /*legacy_grid=*/false);
}
...
std::vector<EntityRef> &SpatialGrid::queryRect(float x1, float y1, float x2,
  ...
  int32_t minX = static_cast<int32_t>(std::floor(x1 * inv_cell_size_));
  int32_t minY = static_cast<int32_t>(std::floor(y1 * inv_cell_size_));
  int32_t maxX = static_cast<int32_t>(std::floor(x2 * inv_cell_size_));
  int32_t maxY = static_cast<int32_t>(std::floor(y2 * inv_cell_size_));

  // Entities are indexed by their top-left corner, so look a few cells up and
  // left to catch sprites that start outside the rect but overlap it.
  minX -= query_pad_cells_;
  minY -= query_pad_cells_;

  minX = std::clamp(minX, 0, cells_w_ - 1);
  minY = std::clamp(minY, 0, cells_h_ - 1);
  maxX = std::clamp(maxX, 0, cells_w_ - 1);
  maxY = std::clamp(maxY, 0, cells_h_ - 1);

  for (int32_t cy = minY; cy <= maxY; ++cy) {
    const int32_t row_base = cy * cells_w_;
  ...
  for (size_t type_index = 0;
    ...
    if (engine->entityManager.getRuntimeKind(static_cast<int>(type_index)) ==
      ...
      for (uint32_t slot = 0; slot < static_cast<uint32_t>(container->count);
        ...
        update_cell_coords(*this, container, slot, container->x_positions[slot],
                           container->y_positions[slot]);
      }
      continue;
    }
