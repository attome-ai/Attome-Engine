...
void ATMByteBufferRunAllBenchmarks() {
  ...
  {
    std::vector<uint8_t> buffer(BENCHMARK_ITERATIONS *
                                8); // Pre-allocate buffer
    ByteBufferWriter writer(buffer);
    ...
    std::cout << "Write int32 (fast) - " << BENCHMARK_ITERATIONS
    ...
    std::cout << "Read int32 (fast) - " << BENCHMARK_ITERATIONS
              << " iterations: " << duration << " microseconds ("
              << static_cast<double>(BENCHMARK_ITERATIONS) /
                     (duration / 1000000.0)
              << " ops/sec)\n";
  ...
  // Benchmark blocks
  {
    ...
    std::generate(sourceData.begin(), sourceData.end(),
                  [&gen]() { return static_cast<uint8_t>(gen() % 256); });
    ...
    std::cout << "Write " << LARGE_SIZE << " bytes (fast) - "
              << LARGE_BENCHMARK_ITERATIONS << " iterations: " << duration
              << " microseconds ("
              << static_cast<double>(LARGE_BENCHMARK_ITERATIONS) /
                     (duration / 1000000.0)
              << " ops/sec)\n";
    ...
    std::cout << "Read " << LARGE_SIZE << " bytes (fast) - "
              << LARGE_BENCHMARK_ITERATIONS << " iterations: " << duration
              << " microseconds ("
              << static_cast<double>(LARGE_BENCHMARK_ITERATIONS) /
                     (duration / 1000000.0)
  ...
  // Benchmark comparison: Standard vs Fast methods
  {
    ...
    std::cout << "Write int32 (standard) - " << BENCHMARK_ITERATIONS
              << " iterations: " << duration << " microseconds ("
              << static_cast<double>(BENCHMARK_ITERATIONS) /
                     (duration / 1000000.0)
              << " ops/sec)\n";
