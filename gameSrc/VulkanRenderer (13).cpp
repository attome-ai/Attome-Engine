...
void VulkanRenderer::updateBatchBuffers() {
    ...
    for (auto& batch : batches) {
        ...
        if (batch.vertexBuffer == VK_NULL_HANDLE) {
            ...
            vkMapMemory(device, batch.vertexMemory, 0,
                sizeof(VulkanVertex) * VulkanConfig::MAX_VERTICES, 0, &batch.mappedVertexMemory);
        }
        ...
        if (batch.indexBuffer == VK_NULL_HANDLE) {
            ...
            // Map the memory once and store the pointer
            vkMapMemory(device, batch.indexMemory, 0,
                sizeof(uint32_t) * VulkanConfig::MAX_INDICES, 0, &batch.mappedIndexMemory);
        }

        // Copy vertex data directly to the persistently mapped memory
        if (batch.vertexBuffer != VK_NULL_HANDLE && batch.mappedVertexMemory != nullptr) {
            memcpy(batch.mappedVertexMemory, batch.vertices, vertexBufferSize);
        }

        // Copy index data directly to the persistently mapped memory
        if (batch.indexBuffer != VK_NULL_HANDLE && batch.mappedIndexMemory != nullptr) {
            memcpy(batch.mappedIndexMemory, batch.indices, indexBufferSize);
        }
