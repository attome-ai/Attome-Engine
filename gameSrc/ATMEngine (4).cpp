    ...
    for (uint32_t containerIdx = 0; containerIdx < engine->entityManager.containers.size(); containerIdx++) {
        ...
        if (entityCount > CHUNK_SIZE) {
                ...
                futures.push_back(std::async(std::launch::async,
                    ...
                    y_positions = container->y_positions]() {
                        ...
                        for (uint32_t i = startIdx; i < endIdx; i++) {
                            entries.push_back({
                                {containerIdx, i},
                                (uint16_t)(x_positions[i] * INV_GRID_CELL_SIZE),
            ...
            for (uint32_t i = 0; i < entityCount; i++) {
                add({ containerIdx, i },
                    container->x_positions[i],
                    container->y_positions[i]);
            }
        }
    }

    // Collect and merge results from all futures - this is now lockless
