...
VulkanRenderer::VulkanRenderer(Engine *engine)
    : engine(engine), instance(VK_NULL_HANDLE), debugMessenger(VK_NULL_HANDLE),
      ...
      swapchainImageFormat(VK_FORMAT_UNDEFINED), renderPass(VK_NULL_HANDLE),
      descriptorSetLayout(VK_NULL_HANDLE), pipelineLayout(VK_NULL_HANDLE),
      graphicsPipeline(VK_NULL_HANDLE), commandPool(VK_NULL_HANDLE),
      descriptorPool(VK_NULL_HANDLE), uniformBuffer(VK_NULL_HANDLE),
      uniformBufferMemory(VK_NULL_HANDLE), currentFrame(0),
      framebufferResized(false), globalVertexBuffer(VK_NULL_HANDLE),
      globalVertexMemory(VK_NULL_HANDLE), mappedGlobalVertexMemory(nullptr),
      globalIndexBuffer(VK_NULL_HANDLE), globalIndexMemory(VK_NULL_HANDLE),
      mappedGlobalIndexMemory(nullptr), currentVertexOffset(0),
      currentIndexOffset(0) {

  // Get window dimensions
  SDL_GetWindowSize(engine->window, &windowWidth, &windowHeight);
  swapchainExtent = {static_cast<uint32_t>(windowWidth),
...
VulkanRenderer::~VulkanRenderer() {
  ...
  // In destructor (add at the beginning of the existing cleanup)
  if (globalVertexBuffer != VK_NULL_HANDLE) {
    vkDestroyBuffer(device, globalVertexBuffer, nullptr);
  }
  if (globalVertexMemory != VK_NULL_HANDLE) {
    if (mappedGlobalVertexMemory != nullptr) {
      vkUnmapMemory(device, globalVertexMemory);
    }
  ...
  if (globalIndexMemory != VK_NULL_HANDLE) {
    if (mappedGlobalIndexMemory != nullptr) {
      vkUnmapMemory(device, globalIndexMemory);
    }
...
bool VulkanRenderer::initialize() {
  ...
  std::cerr << "Log: Creating Vulkan Instance..." << std::endl;
  if (!createInstance()) {
  ...
  std::cerr << "Log: Creating Surface..." << std::endl;

  if (!createSurface()) {
    std::cerr << "Failed to create window surface!" << std::endl;
    return false;
  }
  std::cerr << "Log: Picking Physical Device..." << std::endl;

  if (!pickPhysicalDevice()) {
    std::cerr << "Failed to find a suitable GPU!" << std::endl;
    return false;
  }
  std::cerr << "Log: Creating Logical Device..." << std::endl;
  ...
  // Add to initialize() function before return true
  if (!initializeGlobalBuffers()) {
    std::cerr << "Failed to initialize global buffers!" << std::endl;
    return false;
  }
  return true;
}
...
bool VulkanRenderer::createInstance() {
  ...
  if (VulkanConfig::ENABLE_VALIDATION_LAYERS) {
    createInfo.enabledLayerCount =
        static_cast<uint32_t>(VulkanConfig::validationLayers.size());
...
bool VulkanRenderer::createSurface() {
  ...
  if (SDL_Vulkan_CreateSurface(engine->window, instance, nullptr, &surface) !=
      true) {
...
bool VulkanRenderer::createLogicalDevice() {
  ...
  createInfo.enabledExtensionCount =
      static_cast<uint32_t>(VulkanConfig::deviceExtensions.size());
...
bool VulkanRenderer::createSyncObjects() {
  ...
  fenceInfo.flags =
      VK_FENCE_CREATE_SIGNALED_BIT; // Start signaled so we don't wait forever
...
// Modified VulkanBatch::addQuad method
// Modified addQuad to write directly to global memory with proper offsets
void VulkanBatch::addQuad(float x, float y, float w, float h,
                          SDL_FRect texRegion, VulkanVertex *globalVertices,
                          uint32_t *globalIndices) {

  // Calculate current position in global buffers
  uint32_t localVertexBase = vertexCount;
  uint32_t localIndexBase = indexCount;
  uint32_t globalVertexPos = vertexOffset + localVertexBase;
  uint32_t globalIndexPos = indexOffset + localIndexBase;

  // Pack color (white) - 0xAABBGGRR format (RGBA8)
  ...
  // Write vertices directly to global memory
  // Top-left vertex (0)
  globalVertices[globalVertexPos].setPosition(x, y);
  globalVertices[globalVertexPos].setTexCoord(texRegion.x, texRegion.y);
  globalVertices[globalVertexPos].color = packedColor;

  // Top-right vertex (1)
  globalVertices[globalVertexPos + 1].setPosition(xRight, y);
  globalVertices[globalVertexPos + 1].setTexCoord(texRight, texRegion.y);
  globalVertices[globalVertexPos + 1].color = packedColor;

  // Bottom-right vertex (2)
  globalVertices[globalVertexPos + 2].setPosition(xRight, yBottom);
  globalVertices[globalVertexPos + 2].setTexCoord(texRight, texBottom);
  globalVertices[globalVertexPos + 2].color = packedColor;

  // Bottom-left vertex (3)
  globalVertices[globalVertexPos + 3].setPosition(x, yBottom);
  globalVertices[globalVertexPos + 3].setTexCoord(texRegion.x, texBottom);
  globalVertices[globalVertexPos + 3].color = packedColor;

  // Add indices for two triangles (6 indices total)
  // Important: We need to add vertexOffset to each index since they're local
  // within the batch
  globalIndices[globalIndexPos] = localVertexBase + vertexOffset;
  globalIndices[globalIndexPos + 1] = localVertexBase + 1 + vertexOffset;
  globalIndices[globalIndexPos + 2] = localVertexBase + 2 + vertexOffset;

  globalIndices[globalIndexPos + 3] = localVertexBase + vertexOffset;
  globalIndices[globalIndexPos + 4] = localVertexBase + 2 + vertexOffset;
  globalIndices[globalIndexPos + 5] = localVertexBase + 3 + vertexOffset;
...
void VulkanRenderer::renderScene() {
...

  {
    ...
    VulkanVertex *globalVertices =
        static_cast<VulkanVertex *>(mappedGlobalVertexMemory);
    uint32_t *globalIndices = static_cast<uint32_t *>(mappedGlobalIndexMemory);
    ...
    // Query visible area with a small margin (reduced from 100 to 50 for
    // optimization)
    ...
    std::vector<EntityRef> &visible_entities =
        engine->grid.queryRect(x1, y1, x2, y2);
    ...
    std::sort(visible_entities.begin(), visible_entities.end(),
              [&entityManager](const EntityRef &a, const EntityRef &b) {
                ...
                bool renderableA = containerA->containerFlag &
                                   (uint8_t)ContainerFlag::RENDERABLE;
                bool renderableB = containerB->containerFlag &
                                   (uint8_t)ContainerFlag::RENDERABLE;
                ...
                RenderableEntityContainer *rContainerA =
                    reinterpret_cast<RenderableEntityContainer *>(containerA);
                RenderableEntityContainer *rContainerB =
                    reinterpret_cast<RenderableEntityContainer *>(containerB);
    ...
    for (const EntityRef &entity : visible_entities) {
      ...
      uint64_t batchKey = (static_cast<uint64_t>(textureId) << 32) |
      ...
      // Get texture region directly from textures array (avoiding function
      // call) Get texture region
      SDL_FRect texRegion = textures[textureId].region;

      // Add quad to current batch with direct memory access
      currentBatch->addQuad(screenX, screenY, width, height, texRegion,
                            globalVertices, globalIndices);
    }
  ...
  {
    PROFILE_SCOPE("RECORD AND SUBMIT");
...
void VulkanRenderer::present(uint32_t imageIndex) {
  // Configure present info with minimal setup
  ...
  // Present and handle result with minimal logic
  VkResult result = vkQueuePresentKHR(presentQueue, &presentInfo);

  if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
    // Just mark for recreation on next frame, don't recreate immediately
...
int VulkanRenderer::registerTexture(SDL_Surface *surface, int x, int y,
  ...
  texture.region = {static_cast<float>(x) / atlas_width,
  ...
  for (size_t i = 0; i < VulkanConfig::MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    if (vkAllocateDescriptorSets(device, &allocInfo,
                                 &texture.descriptorSets[i]) != VK_SUCCESS) {
...
// Modified VulkanRenderer::recordCommandBuffer
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer,
  ...
  VkClearValue clearColor = {
      {{0.1f, 0.1f, 0.1f, 1.0f}}}; // Dark gray background
  ...
  // Bind the global vertex and index buffers only once
  if (globalVertexBuffer != VK_NULL_HANDLE &&
      globalIndexBuffer != VK_NULL_HANDLE) {
    VkBuffer vertexBuffers[] = {globalVertexBuffer};
    VkDeviceSize offsets[] = {0};
    vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
    vkCmdBindIndexBuffer(commandBuffer, globalIndexBuffer, 0,
    ...
    for (const auto &batch : batches) {
      ...
      VkDescriptorSet textureDescriptorSet =
          textures[batch.textureId].descriptorSets[currentFrame];
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                              pipelineLayout, 0, 1, &textureDescriptorSet, 0,
                              nullptr);

      // Draw with correct indexing
      // Third parameter (vertexOffset) should be 0 since we embed offsets in
      // indices directly
      vkCmdDrawIndexed(commandBuffer, batch.indexCount, 1, batch.indexOffset, 0,
                       0);
    }
  }
...
bool VulkanRenderer::createTextureImage(SDL_Surface *surface,
                                        VulkanTexture &texture) {
  PROFILE_FUNCTION();
  ...
  if (surface->format != SDL_PIXELFORMAT_RGBA32) {
    ...
    if (!rgbaSurface) {
      std::cerr << "Failed to convert surface to RGBA format: "
  ...
  if (!createImage(texture.width, texture.height, VK_FORMAT_R8G8B8A8_UNORM,
...
bool VulkanRenderer::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  bool extensionsSupported = [device]() {
    ...
    std::set<std::string> requiredExtensions(
        VulkanConfig::deviceExtensions.begin(),
...
// Modified VulkanRenderer::getBatch method
// Modified getBatch method to properly assign memory regions
VulkanBatch &VulkanRenderer::getBatch(int textureId, int zIndex) {
  ...
  uint64_t key =
      (static_cast<uint64_t>(textureId) << 32) | static_cast<uint64_t>(zIndex);
  ...
  size_t newIndex = batches.size();
  batches.emplace_back(textureId, zIndex);
  VulkanBatch &newBatch = batches[newIndex];

  // Set the offsets for this batch in global buffers
  newBatch.vertexOffset = currentVertexOffset;
  newBatch.indexOffset = currentIndexOffset;

  // Update global offsets for next batch
  currentVertexOffset += VulkanConfig::MAX_VERTICES;
  currentIndexOffset += VulkanConfig::MAX_INDICES;
...
bool VulkanRenderer::initializeGlobalBuffers() {
  PROFILE_SCOPE("initializeGlobalBuffers");

  // Calculate buffer sizes with growth room
  // We'll allow for at least MAX_DESCRIPTOR_SETS batches
  const uint32_t maxBatches = VulkanConfig::MAX_DESCRIPTOR_SETS;
  VkDeviceSize vertexBufferSize =
      sizeof(VulkanVertex) * VulkanConfig::MAX_VERTICES * maxBatches;
  VkDeviceSize indexBufferSize =
      sizeof(uint32_t) * VulkanConfig::MAX_INDICES * maxBatches;
  ...
  // Map vertex memory
  vkMapMemory(device, globalVertexMemory, 0, vertexBufferSize, 0,
              &mappedGlobalVertexMemory);
  ...
  if (!createBuffer(indexBufferSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
    ...
    // Clean up vertex resources on failure
    if (mappedGlobalVertexMemory != nullptr) {
      vkUnmapMemory(device, globalVertexMemory);
      mappedGlobalVertexMemory = nullptr;
    }

    vkDestroyBuffer(device, globalVertexBuffer, nullptr);
    vkFreeMemory(device, globalVertexMemory, nullptr);
    globalVertexBuffer = VK_NULL_HANDLE;
    globalVertexMemory = VK_NULL_HANDLE;

    return false;
  }

  // Map index memory
  vkMapMemory(device, globalIndexMemory, 0, indexBufferSize, 0,
              &mappedGlobalIndexMemory);
