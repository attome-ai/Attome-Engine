...
#define NUM_OBSTACLES 1000000  // Reduced from 1,000,000 for better performance
...
int main(int argc, char* argv[]) {
    ...
    while (!quit) {
        ...
        if (game_state.player_entity_index != INVALID_ID) {
            ...
            EntityRef player_ref = { ENTITY_TYPE_PLAYER, static_cast<int32_t>(game_state.player_entity_index) };
        }
...
void setup_game(Engine* engine, GameState* game_state) {
    ...
    if (player_idx != INVALID_ID) {
        ...
        EntityRef player_ref = { ENTITY_TYPE_PLAYER, static_cast<int32_t>(player_idx) };
        // Add to grid
        engine->grid.add(player_ref, start_x, start_y);
        ...
        for (int grid_x = 0; grid_x < dist_grid_width; ++grid_x) {
            ...
            for (int i = 0; i < cell_obstacles; ++i) {
                ...
                if (obstacle_idx != INVALID_ID) {
                    ...
                    EntityRef obstacle_ref = { ENTITY_TYPE_OBSTACLE, static_cast<int32_t>(obstacle_idx) };
                    engine->grid.add(obstacle_ref, x_pos, y_pos);
    ...
    for (int i = 0; i < NUM_ITEMS; ++i) {
        ...
        if (item_idx != INVALID_ID) {
            ...
            EntityRef item_ref = { ENTITY_TYPE_ITEM, static_cast<int32_t>(item_idx) };
            engine->grid.add(item_ref, x_pos, y_pos);
