...
// Modified radix sort for entity indices - now sorts by z_index first, then texture_id
...
void radix_sort_entities(int* indices, int count, EntityManager* entities) {
    ...
    // Use static thread-local buffers to avoid repeated allocations
    static thread_local std::vector<int> s_temp_indices;
    static thread_local std::vector<uint32_t> s_keys;
    static thread_local std::vector<uint32_t> s_temp_keys;

    // Only resize if needed
    if (s_temp_indices.size() < count) {
        s_temp_indices.resize(count);
        s_keys.resize(count);
        s_temp_keys.resize(count);
    }

    int* temp_indices = s_temp_indices.data();
    uint32_t* keys = s_keys.data();
    uint32_t* temp_keys = s_temp_keys.data();

    // Compute keys once to avoid repeated chunk lookups
#pragma omp parallel for if(count > 1000)
    for (int i = 0; i < count; i++) {
        ...
        EntityChunk* chunk = entities->chunks[chunk_idx];
        // Combine z_index and texture_id into a single sortable key
        keys[i] = ((uint32_t)chunk->z_index[local_idx] << 16) | (chunk->texture_id[local_idx] & 0xFFFF);
    }

    // Radix sort implementation optimized for the composite keys
    const int RADIX_BITS = 8;
    const int RADIX_SIZE = 1 << RADIX_BITS;
    const int RADIX_MASK = RADIX_SIZE - 1;
    const int NUM_PASSES = 4; // 32 bits / 8 bits per pass = 4 passes

    int count_bucket[RADIX_SIZE];

    // Sort by byte, from least significant to most significant
    for (int pass = 0; pass < NUM_PASSES; pass++) {
        int shift = pass * RADIX_BITS;

        // Clear counters
        memset(count_bucket, 0, sizeof(count_bucket));
        ...
        // Calculate prefix sum with better cache locality
        int total = 0;
        for (int i = 0; i < RADIX_SIZE; i++) {
            int tmp = count_bucket[i];
            count_bucket[i] = total;
            total += tmp;
        }

        // Reorder data - optimized to reduce cache misses
        for (int i = 0; i < count; i++) {
            uint32_t key = keys[i];
            int bucket = (key >> shift) & RADIX_MASK;
            int pos = count_bucket[bucket]++;
        ...
        memcpy(keys, temp_keys, count * sizeof(uint32_t));
    }
}
...
Engine* engine_create(int window_width, int window_height, int world_width, int world_height, int cell_size) {
    ...
    new (&engine->pending_removals) std::vector<int>();
...
inline void spatial_grid_add(SpatialGrid* grid, int entity_idx, float x, float y) {
    PROFILE_FUNCTION();

    // Use multiplication instead of division (much faster)
    int grid_x = (int)(x * INV_GRID_CELL_SIZE);
    int grid_y = (int)(y * INV_GRID_CELL_SIZE);
    ...
    if (count >= capacity) {
        // Start with larger capacity to reduce reallocations
        int new_capacity = capacity == 0 ? 32 : capacity * 2;
        int* new_cell = static_cast<int*>(SDL_aligned_alloc(CACHE_LINE_SIZE, new_capacity * sizeof(int)));
...
inline void rebuild_spatial_grid(Engine* engine)
{
    PROFILE_SCOPE("rebuild_spatial_grid");
    SpatialGrid* grid = &engine->grid;

    // Clear the grid by resetting all counts to 0
    for (int y = 0; y < grid->height; ++y) {
        memset(grid->cell_counts[y], 0, grid->width * sizeof(int));
    }

    for (int chunk_idx = 0; chunk_idx < engine->entities.chunk_count; chunk_idx++) {
        EntityChunk* chunk = engine->entities.chunks[chunk_idx];
        const int chunk_base_idx = chunk_idx * ENTITY_CHUNK_SIZE;

        // Non-SIMD version with optimizations
        for (int local_idx = 0; local_idx < chunk->count; local_idx++) {
            if (chunk->visible[local_idx]) {
                int entity_idx = chunk_base_idx + local_idx;
                spatial_grid_add(grid, entity_idx, chunk->x[local_idx], chunk->y[local_idx]);
            }
        }
    }
}
...
int engine_register_entity_type(Engine* engine, EntityTypeUpdateFunc update_func, size_t extra_data_size) {
    PROFILE_FUNCTION();

    // Generate a new type ID
    int type_id = engine->next_type_id++;
    ...
    // Ensure the vector is large enough for the new type_id
    if (type_id >= engine->type_id_to_index.size()) {
        engine->type_id_to_index.resize(type_id + 1, -1);  // Initialize new entries with -1
    }
    ...
    config->first_chunk_idx = -1;

    return type_id;  // Return the generated type ID
}
...
int find_or_create_chunk_for_type(Engine* engine, int type_id) {
    ...
    // Get the type index from the vector
    if (type_id < 0 || type_id >= engine->type_id_to_index.size() ||
        engine->type_id_to_index[type_id] == -1) {
        // Type not registered
        return -1;
    }

    int type_index = engine->type_id_to_index[type_id];
    EntityTypeConfig* config = &engine->entity_types[type_index];
...
// Add entity with a specific type - updated to include z_index parameter with default value
int engine_add_entity_with_type(Engine* engine, int type_id, float x, float y,
    int width, int height, int texture_id, int layer, int z_index) {
    PROFILE_FUNCTION();
    ...
    chunk->z_index[local_idx] = z_index;  // Set z_index
...
void* engine_get_entity_type_data(Engine* engine, int entity_idx) {
    ...
    // Use the vector for type lookup
...
// Forward declare the global engine pointer for Emscripten
#ifdef __EMSCRIPTEN__
Engine* g_current_engine = nullptr;
bool g_processing_complete = true; // Flag to track if processing is complete

// Function to process a single chunk (called from JS)
extern "C" {
    EMSCRIPTEN_KEEPALIVE
        void EMSCRIPTEN_ProcessChunk(int chunk_idx, float delta_time) {
        if (!g_current_engine || chunk_idx >= g_current_engine->entities.chunk_count) return;

        auto chunk = g_current_engine->entities.chunks[chunk_idx];
        if (!chunk) return;

        auto config = &g_current_engine->entity_types[chunk->type_id];
        config->update_func(chunk, chunk->count, delta_time);
    }

    // Function to mark processing as complete
    EMSCRIPTEN_KEEPALIVE
        void EMSCRIPTEN_MarkProcessingComplete() {
...
void engine_update_entity_types(Engine* engine, float delta_time) {
...
#ifdef __EMSCRIPTEN__
    // If previous processing is still ongoing, process synchronously
    // to avoid piling up async tasks
    if (!g_processing_complete) {
        // Fallback to synchronous processing for this frame
        for (int i = 0; i < engine->entities.chunk_count; i++) {
            auto chunk = engine->entities.chunks[i];
            auto config = &engine->entity_types[chunk->type_id];
            config->update_func(chunk, chunk->count, delta_time);
        }
        return;
    }

    // Mark as processing has started
    g_processing_complete = false;

    // Store engine pointer for async callbacks
    g_current_engine = engine;

    // If no entities to process, mark as complete and return
    if (engine->entities.chunk_count == 0) {
        g_processing_complete = true;
        return;
    }

    // Use a fixed batch size to balance responsiveness and performance
    int BATCH_SIZE = 5; // Process 5 chunks at a time

    // Set up async processing using a cleaner approach
    EM_ASM({
        // Clear any existing processing state
        if (window.entityProcessingTimeout) {
            clearTimeout(window.entityProcessingTimeout);
            window.entityProcessingTimeout = null;
        }
        ...
        function processBatch() {
            ...
            var endChunk = Math.min(currentChunk + batchSize, chunkCount);

            for (var i = currentChunk; i < endChunk; i++) {
                _EMSCRIPTEN_ProcessChunk(i, deltaTime);
            }

            currentChunk = endChunk;

            if (currentChunk < chunkCount) {
                // Schedule next batch with a short delay to yield to browser
                window.entityProcessingTimeout = setTimeout(processBatch, 0);
            }
 else {
                // All chunks processed, mark as complete
                window.entityProcessingTimeout = null;
                _EMSCRIPTEN_MarkProcessingComplete();
            }
        }

        // Start processing immediately
        processBatch();
        }, engine->entities.chunk_count, delta_time, BATCH_SIZE);

#else
    // Multi-threaded path for native platforms
    const int MAX_THREADS = 12;
    const int num_tasks = std::min(MAX_THREADS, engine->entities.chunk_count);

    // Early return for single-threaded case
    if (num_tasks <= 1) {
        for (int i = 0; i < engine->entities.chunk_count; i++) {
            auto chunk = engine->entities.chunks[i];
            auto config = &engine->entity_types[chunk->type_id];
    ...
    for (int task_id = 0; task_id < num_tasks; task_id++) {
        futures.push_back(std::async(std::launch::async, [engine, delta_time, task_id, num_tasks]() {
            // Process chunks assigned to this task
            for (int i = task_id; i < engine->entities.chunk_count; i += num_tasks) {
                auto chunk = engine->entities.chunks[i];
                auto config = &engine->entity_types[chunk->type_id];
                config->update_func(chunk, chunk->count, delta_time);
            }
...
// Helper function to check if entity updates are complete
// This can be used in the main loop to determine if it's safe to continue
bool engine_entity_updates_complete() {
#ifdef __EMSCRIPTEN__
    return g_processing_complete;
#else
    return true; // Always complete for non-Emscripten (synchronous) version
#endif
}

// Function to mark an entity for removal
void engine_remove_entity(Engine* engine, int entity_idx) {
    // Add to pending removals - will be processed after rendering
    engine->pending_removals.push_back(entity_idx);
}

// Process all entities marked for removal
void process_pending_removals(Engine* engine) {
    if (engine->pending_removals.empty())
        return;

    // Sort indices in descending order so that higher indices are processed first
    // This ensures we don't invalidate indices we still need to process
    std::sort(engine->pending_removals.begin(), engine->pending_removals.end(),
        [](int a, int b) { return a > b; });

    // Process each pending removal
    for (int entity_idx : engine->pending_removals) {
        // Get chunk and local index
        int chunk_idx, local_idx;
        engine->entities.getChunkIndices(entity_idx, &chunk_idx, &local_idx);

        // Check if chunk index is valid
        if (chunk_idx < 0 || chunk_idx >= engine->entities.chunk_count) {
        ...
        // If this is already the last entity, just decrement count
        if (local_idx == chunk->count - 1) {
            chunk->count--;
            continue;
        }

        // Get the last entity in the chunk
        int last_idx = chunk->count - 1;

        // Swap all properties with the last entity in chunk
        chunk->x[local_idx] = chunk->x[last_idx];
        chunk->y[local_idx] = chunk->y[last_idx];
        chunk->right[local_idx] = chunk->right[last_idx];
        chunk->bottom[local_idx] = chunk->bottom[last_idx];
        chunk->width[local_idx] = chunk->width[last_idx];
        chunk->height[local_idx] = chunk->height[last_idx];
        chunk->texture_id[local_idx] = chunk->texture_id[last_idx];
        chunk->layer[local_idx] = chunk->layer[last_idx];
        chunk->z_index[local_idx] = chunk->z_index[last_idx];
        chunk->visible[local_idx] = chunk->visible[last_idx];
        chunk->grid_cell[local_idx] = chunk->grid_cell[last_idx];

        // Swap hierarchy properties
        chunk->parent_id[local_idx] = chunk->parent_id[last_idx];
        chunk->first_child_id[local_idx] = chunk->first_child_id[last_idx];
        chunk->next_sibling_id[local_idx] = chunk->next_sibling_id[last_idx];
        chunk->local_x[local_idx] = chunk->local_x[last_idx];
        chunk->local_y[local_idx] = chunk->local_y[last_idx];

        // Swap type-specific data if any
        if (chunk->type_data) {
            // Get type index for the chunk's type
            int type_id = chunk->type_id;
            if (type_id >= 0 && type_id < engine->type_id_to_index.size()) {
                int type_index = engine->type_id_to_index[type_id];
                size_t extra_data_size = engine->entity_types[type_index].extra_data_size;
                ...
                if (extra_data_size > 0) {
                    uint8_t* typed_data = static_cast<uint8_t*>(chunk->type_data);
                    uint8_t* dst = typed_data + (local_idx * extra_data_size);
                    uint8_t* src = typed_data + (last_idx * extra_data_size);
                    memcpy(dst, src, extra_data_size);
                }
            }
        }
