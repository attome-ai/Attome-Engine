...
void SpatialGrid::rebuild_grid(Engine* engine) {
    PROFILE_FUNCTION();
    clearAll();
    constexpr uint32_t CHUNK_SIZE = 25000;
    ...
    for (uint32_t containerIdx = 0; containerIdx < engine->entityManager.containers.size(); containerIdx++) {
        ...
        if (entityCount > CHUNK_SIZE) {
            ...
            for (uint32_t chunk = 0; chunk < numChunks; chunk++) {
                ...
                engine->taskSystem->submit([this, containerIdx, startIdx, endIdx,
                    x_positions = container->x_positions,
    ...
    engine->taskSystem->waitForCompletion();

};
#include <windows.h> // Added Windows header
#undef min

#undef max

// Function to set thread priority and affinity
void TaskSystem::setThreadPriorityAndAffinity(uint32_t threadIndex, uint32_t threadCount) {
