...
VulkanRenderer::VulkanRenderer(Engine* engine)
    ...
    globalIndexMemory(VK_NULL_HANDLE),
    mappedGlobalIndexMemory(nullptr),
...
void VulkanBatch::addQuad(float x, float y, float w, float h, SDL_FRect texRegion,
    ...
    // Important: We need to add vertexOffset to each index since they're local within the batch
...
void VulkanRenderer::renderScene() {
...

    {
        ...
        VulkanVertex* globalVertices = static_cast<VulkanVertex*>(mappedGlobalVertexMemory);
...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    if (globalVertexBuffer != VK_NULL_HANDLE && globalIndexBuffer != VK_NULL_HANDLE) {
        ...
        for (const auto& batch : batches) {
            ...
            // Third parameter (vertexOffset) should be 0 since we embed offsets in indices directly
            vkCmdDrawIndexed(commandBuffer, batch.indexCount, 1, batch.indexOffset, 0, 0);
        }
    }
...
bool VulkanRenderer::initializeGlobalBuffers() {
    ...
    VkDeviceSize vertexBufferSize = sizeof(VulkanVertex) * VulkanConfig::MAX_VERTICES * maxBatches;
    VkDeviceSize indexBufferSize = sizeof(uint32_t) * VulkanConfig::MAX_INDICES * maxBatches;
    ...
    // Map vertex memory
    vkMapMemory(device, globalVertexMemory, 0, vertexBufferSize, 0, &mappedGlobalVertexMemory);
    ...
    // Map index memory
    vkMapMemory(device, globalIndexMemory, 0, indexBufferSize, 0, &mappedGlobalIndexMemory);
