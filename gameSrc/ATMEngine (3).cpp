...
void RenderBatch::addQuad(float x, float y, float w, float h, SDL_FRect tex_region) {
    ...
    v[1].color.b = 1;
    v[1].color.r = 1;
    v[1].color.g = 1;    v[1].tex_coord.x = tex_region.x + tex_region.w;
    ...
    v[2].color.b = 1;
    v[2].color.r = 1;
    v[2].color.g = 1;    v[2].tex_coord.x = tex_region.x + tex_region.w;
    ...
    v[3].color.b = 1;
    v[3].color.r = 1;
    v[3].color.g = 1;    v[3].tex_coord.x = tex_region.x;
...
RenderBatchManager::BatchKey RenderBatchManager::createKey(int textureId, int zIndex) {
    PROFILE_FUNCTION();
...
void engine_render_scene(Engine* engine) {
...

    {
        ...
        const std::vector<EntityRef>& visible_entities = engine->grid.queryRect(
            x1, y1, x2, y2);
...
#include <future>
// Helper struct to store grid entries
struct GridEntry {
    EntityRef entityId;
    uint16_t x;
    uint16_t y;
...
void SpatialGrid::rebuild_grid(Engine* engine) {
    ...
    constexpr uint32_t CHUNK_SIZE = 10000; // Adjust based on your needs
    static std::vector<std::future<std::vector<GridEntry>>> futures;
    futures.clear();
    ...
    for (uint32_t containerIdx = 0; containerIdx < engine->entityManager.containers.size(); containerIdx++) {
        ...
        // If container has enough entities, split into chunks
        if (entityCount > CHUNK_SIZE) {
            ...
            for (uint32_t chunk = 0; chunk < numChunks; chunk++) {
                ...
                futures.push_back(std::async(std::launch::async,
                    [containerIdx, startIdx, endIdx, x_positions = container->x_positions,
                    y_positions = container->y_positions]() {
                        std::vector<GridEntry> entries;
                        entries.reserve(endIdx - startIdx);

                        for (uint32_t i = startIdx; i < endIdx; i++) {
                            entries.push_back({
                                {containerIdx, i},(uint16_t)(x_positions[i]* INV_GRID_CELL_SIZE),
                            (uint16_t)(y_positions[i]* INV_GRID_CELL_SIZE)
                                });
                        }
                        return entries;
                    }));
            ...
            // Process small containers directly
            for (uint32_t y = 0; y < entityCount; y++) {
                add({ containerIdx, y },
                    container->x_positions[y],
                    container->y_positions[y]);
            }
        }
    }
    ...
    for (auto& future : futures) {
        ...
        for (const auto& entry : entries) {
            add2(entry.entityId, entry.x, entry.y);
        }
    }
}
...
void SpatialGrid::printGrid() const {
    std::cout << "Grid Layout (entities per cell, 8x8):\n";
    ...
    // Print grid content (only first 8x8)
    for (size_t y = 0; y < 8; ++y) {
        std::cout << y << "\t|";
        for (size_t x = 0; x < 8; ++x) {
            std::cout << "\t" << cells[x][y].entities.size();
        }
        std::cout << "\n";
    }
    std::cout << std::endl;
...
void SpatialGrid::printQueryRect(float minX, float minY, float maxX, float maxY) {
    ...
    std::cout << "Rectangle Query (cells " << minCellX << "," << minCellY
        << " to " << maxCellX << "," << maxCellY << "):\n";

    // Print column headers (only affected range)
    std::cout << "   ";
...
void SpatialGrid::printQueryCircle(float centerX, float centerY, float radius) {
    ...
    std::cout << "Circle Query (center: " << centerX << "," << centerY
        << ", radius: " << radius << "):\n";
    ...
    for (size_t y = minCellY; y <= maxCellY; ++y) {
        std::cout << std::setw(2) << y << "|";
        for (size_t x = minCellX; x <= maxCellX; ++x) {
            float cellWorldX = x * GRID_CELL_SIZE;
            float cellWorldY = y * GRID_CELL_SIZE;
