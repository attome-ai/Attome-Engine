...
void VulkanRenderer::renderScene() {
    ...
    {
        PROFILE_SCOPE("waiting for Fence");
    ...
    {
        PROFILE_SCOPE("Aquire Image KHR");
    ...
    {
        PROFILE_SCOPE("waitForImageFence");

        // If this image has a fence associated with it, wait for it to be signaled
        // This ensures the image is not currently in use before we use it again
        ...
        // Don't reset the fence yet - we'll do that when submitting the command buffer
        imageFences[imageIndex] = inFlightFences[currentFrame];
    }

    // Reset the fence for this frame now that we're about to submit new work
    ...
    {
        PROFILE_SCOPE("extractVisibleEntities");
    ...
    {
        PROFILE_SCOPE("updateBatchBuffers");
    ...
    {
        PROFILE_SCOPE("recordCommandBuffer");
    ...
    {
        PROFILE_SCOPE("submitCommandBuffer");
        ...
        // Submit with this frame's fence - will be signaled when the GPU completes this work
...
void VulkanRenderer::updateBatchBuffers() {
    ...
    // Second pass: Create/update all buffers
    for (auto& batch : batches) {
        if (batch.vertices.empty() || !batch.needsUpdate) {
            continue;
        }
        ...
        // ---- VERTEX BUFFER ----
        // Only recreate if NULL or needs to grow - don't shrink
        if (batch.vertexBuffer == VK_NULL_HANDLE || batch.vertices.size() > batch.vertices.capacity()) {
            ...
            size_t newCapacity = std::max((uint32_t)batch.vertices.size(), VulkanConfig::INITIAL_BATCH_VERTEX_CAPACITY);
            newCapacity *= 2; // Double for future growth

            // Create staging buffer (host visible)
            if (!createBuffer(
                sizeof(VulkanVertex) * newCapacity,
        ...
        // Copy data to vertex staging buffer if it exists
        if (batch.vertexStagingBuffer != VK_NULL_HANDLE) {
            void* data;
        ...
        // ---- INDEX BUFFER ----
        // Only recreate if NULL or needs to grow - don't shrink
        if (batch.indexBuffer == VK_NULL_HANDLE || batch.indices.size() > batch.indices.capacity()) {
            // Make sure all GPU operations are complete before destroying resources
            vkDeviceWaitIdle(device);
            ...
            // Calculate new capacity with room for growth
            size_t newCapacity = std::max(batch.indices.size(), (size_t)(VulkanConfig::INITIAL_BATCH_VERTEX_CAPACITY * 1.5f));
            newCapacity *= 2; // Double for future growth
        ...
        // Copy data to index staging buffer if it exists
        if (batch.indexStagingBuffer != VK_NULL_HANDLE) {
            void* data;
    ...
    // Third pass: Create a single command buffer for all valid transfers
    ...
    // Record copy commands for all batches with valid buffers
    bool hasValidCopyOperations = false;
    ...
    for (auto& batch : batches) {
        ...
        // Skip if any required buffer is null
        if (batch.vertexBuffer == VK_NULL_HANDLE || batch.vertexStagingBuffer == VK_NULL_HANDLE ||
            batch.indexBuffer == VK_NULL_HANDLE || batch.indexStagingBuffer == VK_NULL_HANDLE) {
            continue;
        }

        // We have at least one valid copy operation
        hasValidCopyOperations = true;
    ...
    // Only submit if we have valid copy operations
    if (hasValidCopyOperations) {
        // Use a static fence for transfer operations
        static VkFence transferFence = VK_NULL_HANDLE;

        // Wait for any previous transfer operation to complete before destroying the fence
        if (transferFence != VK_NULL_HANDLE) {
            // Wait for the fence with a timeout
            VkResult result = vkWaitForFences(device, 1, &transferFence, VK_TRUE, UINT64_MAX);
            if (result != VK_SUCCESS) {
                std::cerr << "Failed to wait for transfer fence: " << result << std::endl;
            }

            // Now it's safe to destroy the fence
        ...
        if (vkCreateFence(device, &fenceInfo, nullptr, &transferFence) != VK_SUCCESS) {
            ...
            // Continue without a fence - not ideal but better than failing completely
        }
        ...
        // Free the command buffer if we didn't use it
        vkFreeCommandBuffers(device, commandPool, 1, &transferCommandBuffer);
    }
}

void VulkanRenderer::waitForBatchUpdates() {
    PROFILE_SCOPE("waitForBatchUpdates");

    // Access the same static fence used in updateBatchBuffers
    static VkFence transferFence = VK_NULL_HANDLE;
    ...
    if (transferFence != VK_NULL_HANDLE) {
        ...
        VkResult result = vkWaitForFences(device, 1, &transferFence, VK_TRUE, UINT64_MAX);
        if (result != VK_SUCCESS) {
            std::cerr << "Failed to wait for transfer fence in waitForBatchUpdates: " << result << std::endl;
        }
    }
}