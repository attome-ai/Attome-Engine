...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    if (globalVertexBuffer != VK_NULL_HANDLE && globalIndexBuffer != VK_NULL_HANDLE) {
        ...
        for (const auto& batch : batches) {
            ...
            // Draw with offsets into the global buffers
            vkCmdDrawIndexed(commandBuffer, batch.indexCount, 1, batch.indexOffset, batch.vertexOffset, 0);
        }
    }
...
void VulkanRenderer::updateBatchBuffers() {
    // Calculate total required buffer sizes
    ...
    for (auto& batch : batches) {
        ...
        // Add to total sizes
        totalVertexSize += sizeof(VulkanVertex) * batch.vertexCount;
        totalIndexSize += sizeof(uint32_t) * batch.indexCount;
    }

    // Create global buffers if needed
    if (globalVertexBuffer == VK_NULL_HANDLE) {
        // Create host-visible buffer for all vertex data
        if (!createBuffer(
            sizeof(VulkanVertex) * VulkanConfig::MAX_VERTICES * batches.size(), // Multiply by batch count for worst case
        ...
        // Map the memory once and store the pointer
        vkMapMemory(device, globalVertexMemory, 0,
            sizeof(VulkanVertex) * VulkanConfig::MAX_VERTICES * batches.size(), 0, &mappedGlobalVertexMemory);
    }

    if (globalIndexBuffer == VK_NULL_HANDLE) {
        // Create host-visible buffer for all index data
        if (!createBuffer(
            sizeof(uint32_t) * VulkanConfig::MAX_INDICES * batches.size(), // Multiply by batch count for worst case
        ...
        // Map the memory once and store the pointer
        vkMapMemory(device, globalIndexMemory, 0,
            sizeof(uint32_t) * VulkanConfig::MAX_INDICES * batches.size(), 0, &mappedGlobalIndexMemory);
    }
    ...
    for (auto& batch : batches) {
        ...
        // Copy vertex data
        VkDeviceSize vertexSize = sizeof(VulkanVertex) * batch.vertexCount;
        memcpy(static_cast<char*>(mappedGlobalVertexMemory) + currentVertexOffset * sizeof(VulkanVertex),
            batch.vertices, vertexSize);

        // Copy index data (need to adjust indices to point to the correct vertices)
        for (uint32_t i = 0; i < batch.indexCount; i++) {
            // Adjust indices to point to the correct vertices in the global buffer
            uint32_t adjustedIndex = batch.indices[i] + currentVertexOffset;
            memcpy(static_cast<char*>(mappedGlobalIndexMemory) + (currentIndexOffset + i) * sizeof(uint32_t),
                &adjustedIndex, sizeof(uint32_t));
        }
...
bool VulkanRenderer::initializeGlobalBuffers() {
    ...
    // Calculate buffer sizes with some room for growth
    VkDeviceSize maxVertexBufferSize = sizeof(VulkanVertex) * VulkanConfig::MAX_VERTICES * VulkanConfig::MAX_DESCRIPTOR_SETS;
    VkDeviceSize maxIndexBufferSize = sizeof(uint32_t) * VulkanConfig::MAX_INDICES * VulkanConfig::MAX_DESCRIPTOR_SETS;
    ...
    // Map the memory once and store the pointer
    vkMapMemory(device, globalVertexMemory, 0, maxVertexBufferSize, 0, &mappedGlobalVertexMemory);
    ...
    if (!createBuffer(
        ...
        // Clean up vertex buffer on failure
        vkUnmapMemory(device, globalVertexMemory);
        vkDestroyBuffer(device, globalVertexBuffer, nullptr);
        vkFreeMemory(device, globalVertexMemory, nullptr);
        globalVertexBuffer = VK_NULL_HANDLE;
    ...
    // Map the memory once and store the pointer
    vkMapMemory(device, globalIndexMemory, 0, maxIndexBufferSize, 0, &mappedGlobalIndexMemory);
