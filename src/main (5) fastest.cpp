...
void init_buffer_pool(BufferPool* pool, size_t buffer_size, int initial_capacity) {
    ...
    pool->count = 0;
    pool->buffers = (void**)aligned_malloc(initial_capacity * sizeof(void*), CACHE_LINE_SIZE);
}
...
void* get_buffer(BufferPool* pool) {
    ...
    return aligned_malloc(pool->buffer_size, 32); // 32-byte alignment for AVX
}
...
void return_buffer(BufferPool* pool, void* buffer) {
    if (pool->count >= pool->capacity) {
        ...
        void** new_buffers = (void**)aligned_malloc(new_capacity * sizeof(void*), CACHE_LINE_SIZE);
        memcpy(new_buffers, pool->buffers, pool->count * sizeof(void*));
        aligned_free(pool->buffers);
...
int simd_frustum_cull(EntityManager* entities, SDL_FRect view_rect, int* cell_entities,
    ...
    for (int i = 0; i < cell_entity_count; i += 4) {
        ...
        __m128 test1 = _mm_cmplt_ps(entity_right, view_min_x);  // entity_right < view_min_x
        __m128 test2 = _mm_cmpgt_ps(entity_x, view_max_x);      // entity_x > view_max_x
        __m128 test3 = _mm_cmplt_ps(entity_bottom, view_min_y); // entity_bottom < view_min_y
        __m128 test4 = _mm_cmpgt_ps(entity_y, view_max_y);      // entity_y > view_max_y
    ...
    for (int i = 0; i < cell_entity_count; i += 4) {
        ...
        float entity_coords[16]; // x, y, right, bottom for each entity (4 entities)
        for (int j = 0; j < batch_size; j++) {
            entity_coords[j] = entities->x[indices_batch[j]];
            entity_coords[j + 4] = entities->y[indices_batch[j]];
            entity_coords[j + 8] = entities->right[indices_batch[j]];
            entity_coords[j + 12] = entities->bottom[indices_batch[j]];
        }

        float32x4_t entity_x = vld1q_f32(&entity_coords[0]);
        float32x4_t entity_y = vld1q_f32(&entity_coords[4]);
        float32x4_t entity_right = vld1q_f32(&entity_coords[8]);
        float32x4_t entity_bottom = vld1q_f32(&entity_coords[12]);

        // Check if entities are in view
        uint32x4_t test1 = vcltq_f32(entity_right, view_min_x);  // entity_right < view_min_x
        uint32x4_t test2 = vcgtq_f32(entity_x, view_max_x);      // entity_x > view_max_x
        uint32x4_t test3 = vcltq_f32(entity_bottom, view_min_y); // entity_bottom < view_min_y
        uint32x4_t test4 = vcgtq_f32(entity_y, view_max_y);      // entity_y > view_max_y
...
void init_entity_manager(EntityManager* manager, int initial_capacity) {
    ...
    manager->right = (float*)aligned_malloc(initial_capacity * sizeof(float), CACHE_LINE_SIZE);
    manager->bottom = (float*)aligned_malloc(initial_capacity * sizeof(float), CACHE_LINE_SIZE);
...
void free_entity_manager(EntityManager* manager) {
    ...
    aligned_free(manager->right);
    aligned_free(manager->bottom);
...
int add_entity(GameState* state, float x, float y, int width, int height,
    ...
    if (em->count >= em->capacity) {
        ...
        float* new_right = (float*)aligned_malloc(new_capacity * sizeof(float), CACHE_LINE_SIZE);
        float* new_bottom = (float*)aligned_malloc(new_capacity * sizeof(float), CACHE_LINE_SIZE);
        ...
        aligned_free(em->x);
        aligned_free(em->y);
        aligned_free(em->right);
        aligned_free(em->bottom);
...
void create_batch(RenderBatch** batches, int* batch_count, int texture_id, int layer) {
    ...
    *batches = (RenderBatch*)realloc(*batches, (*batch_count) * sizeof(RenderBatch));
    ...
    (*batches)[new_idx].vertices = (SDL_Vertex*)aligned_malloc(
        (*batches)[new_idx].vertex_capacity * sizeof(SDL_Vertex), CACHE_LINE_SIZE);
    (*batches)[new_idx].indices = (int*)aligned_malloc(
...
void render_game(GameState* state) {
    ...
    float* world_x = (float*)aligned_malloc(visible_count * sizeof(float), 32); // AVX alignment
    float* world_y = (float*)aligned_malloc(visible_count * sizeof(float), 32);
    float* width_f = (float*)aligned_malloc(visible_count * sizeof(float), 32);
    float* height_f = (float*)aligned_malloc(visible_count * sizeof(float), 32);
    ...
    aligned_free(width_f);
    aligned_free(height_f);
