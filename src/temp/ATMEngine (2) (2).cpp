...
#define ENTITY_CHUNK_SIZE 16384  // 16K entities per chunk (power of 2 for bit operations)
#define ENTITY_CHUNK_BITS 14    // 2^14 = 16384, for bitwise operations
#define ENTITY_CHUNK_MASK 0x3FFF // Mask for extracting local index (16384-1)
...
#include <xmmintrin.h>
// rw: 0 = read, 1 = write; locality: 0 = no temporal locality, 3 = high temporal locality
#define PREFETCH(addr, rw, locality) _mm_prefetch((const char*)(addr), (locality == 0) ? _MM_HINT_NTA : _MM_HINT_T0)
#else
#define PREFETCH(addr, rw, locality) ((void)0) // No-op for platforms without prefetch
#endif

// Inline access macros for entity data
#define ENTITY_CHUNK_IDX(idx) ((idx) >> ENTITY_CHUNK_BITS)
#define ENTITY_LOCAL_IDX(idx) ((idx) & ENTITY_CHUNK_MASK)
#define GET_ENTITY_X(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->x[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_Y(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->y[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_RIGHT(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->right[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_BOTTOM(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->bottom[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_WIDTH(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->width[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_HEIGHT(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->height[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_TEXTURE_ID(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->texture_id[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_LAYER(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->layer[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_ACTIVE(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->active[ENTITY_LOCAL_IDX(idx)])
#define GET_ENTITY_GRID_CELL(entities, idx) ((entities)->chunks[ENTITY_CHUNK_IDX(idx)]->grid_cell[ENTITY_LOCAL_IDX(idx)])
...
// Fast bitwise chunk index calculation
void get_chunk_indices(int entity_idx, int* chunk_idx, int* local_idx) {
    *chunk_idx = entity_idx >> ENTITY_CHUNK_BITS;  // Divide by ENTITY_CHUNK_SIZE using right shift
    *local_idx = entity_idx & ENTITY_CHUNK_MASK;   // Modulo ENTITY_CHUNK_SIZE using bitwise AND
}
...
int get_new_entity_index(EntityManager* manager) {
    ...
    int chunk_idx = entity_idx >> ENTITY_CHUNK_BITS; // Bitwise division
    ...
    int local_idx = entity_idx & ENTITY_CHUNK_MASK; // Bitwise modulo
...
int simd_frustum_cull(EntityManager* entities, SDL_FRect view_rect, int* cell_entities,
    ...
    // Prefetch first entities data
    if (cell_entity_count > 0) {
        int entity_idx = cell_entities[0];
        int chunk_idx = ENTITY_CHUNK_IDX(entity_idx);
        EntityChunk* chunk = entities->chunks[chunk_idx];

        PREFETCH(chunk->active, 0, 3);
        PREFETCH(chunk->x, 0, 3);
        PREFETCH(chunk->y, 0, 3);
        PREFETCH(chunk->right, 0, 3);
        PREFETCH(chunk->bottom, 0, 3);
    }
    ...
    for (int i = 0; i < cell_entity_count; i++) {
        ...
        int chunk_idx = ENTITY_CHUNK_IDX(entity_idx);
        int local_idx = ENTITY_LOCAL_IDX(entity_idx);

        // Prefetch next entity data if available
        if (i + 1 < cell_entity_count) {
            int next_idx = cell_entities[i + 1];
            int next_chunk = ENTITY_CHUNK_IDX(next_idx);
            EntityChunk* next_chunk_ptr = entities->chunks[next_chunk];
            PREFETCH(&next_chunk_ptr->active[ENTITY_LOCAL_IDX(next_idx)], 0, 3);
            PREFETCH(&next_chunk_ptr->x[ENTITY_LOCAL_IDX(next_idx)], 0, 3);
            PREFETCH(&next_chunk_ptr->y[ENTITY_LOCAL_IDX(next_idx)], 0, 3);
            PREFETCH(&next_chunk_ptr->right[ENTITY_LOCAL_IDX(next_idx)], 0, 3);
            PREFETCH(&next_chunk_ptr->bottom[ENTITY_LOCAL_IDX(next_idx)], 0, 3);
        }

        EntityChunk* chunk = entities->chunks[chunk_idx];
        bool active = chunk->active[local_idx];

        if (active) {
            float x = chunk->x[local_idx];
            float y = chunk->y[local_idx];
            float right = chunk->right[local_idx];
            float bottom = chunk->bottom[local_idx];
...
void spatial_grid_query(SpatialGrid* grid, SDL_FRect query_rect,
    ...
    *result_count = 0;
    ...
    // Prefetch the first row of cells if available
    if (start_y < end_y && grid->cells[start_y]) {
        PREFETCH(grid->cells[start_y], 0, 3);
        PREFETCH(grid->cell_counts[start_y], 0, 3);
    }
    ...
    for (int y = start_y; y <= end_y && *result_count < max_results; y++) {
        ...
        if (y + 1 <= end_y && grid->cells[y + 1]) {
            PREFETCH(grid->cells[y + 1], 0, 3);
            PREFETCH(grid->cell_counts[y + 1], 0, 3);
        }

        // Skip rows that aren't allocated
        if (!grid->cells[y]) continue;

        // Prefetch first cell in row if available
        if (start_x <= end_x && grid->cells[y][start_x]) {
            PREFETCH(grid->cells[y][start_x], 0, 3);
        }

        for (int x = start_x; x <= end_x && *result_count < max_results; x++) {
            // Prefetch next cell if available
            if (x + 1 <= end_x && grid->cells[y][x + 1]) {
                PREFETCH(grid->cells[y][x + 1], 0, 3);
            }
...
void radix_sort_entities(int* indices, int count, EntityManager* entities) {
    ...
    // Count frequencies - process entities chunk by chunk for better cache coherency
    for (int i = 0; i < count; i++) {
        int entity_idx = indices[i];
        int texture_id = GET_ENTITY_TEXTURE_ID(entities, entity_idx) & RADIX_MASK;
    ...
    // Sort by texture_id - process entities chunk by chunk
    for (int i = 0; i < count; i++) {
        int entity_idx = indices[i];
        int texture_id = GET_ENTITY_TEXTURE_ID(entities, entity_idx) & RADIX_MASK;
    ...
    for (int tex = 0; tex < RADIX_SIZE; tex++) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            int layer = GET_ENTITY_LAYER(entities, entity_idx) & RADIX_MASK;
            count_layer[layer]++;
        }
...
void calculate_screen_coordinates(EntityManager* entities, int* entity_indices,
    ...
    // Prefetch the first entity's data if available
    if (count > 0) {
        int idx = entity_indices[0];
        int chunk_idx = ENTITY_CHUNK_IDX(idx);
        int local_idx = ENTITY_LOCAL_IDX(idx);
        EntityChunk* chunk = entities->chunks[chunk_idx];

        PREFETCH(&chunk->x[local_idx], 0, 3);
        PREFETCH(&chunk->y[local_idx], 0, 3);
        PREFETCH(&chunk->width[local_idx], 0, 3);
        PREFETCH(&chunk->height[local_idx], 0, 3);
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        // Prefetch next entity data if available
        if (i + 1 < count) {
            int next_idx = entity_indices[i + 1];
            int next_chunk = ENTITY_CHUNK_IDX(next_idx);
            int next_local = ENTITY_LOCAL_IDX(next_idx);
            EntityChunk* next_chunk_ptr = entities->chunks[next_chunk];

            PREFETCH(&next_chunk_ptr->x[next_local], 0, 3);
            PREFETCH(&next_chunk_ptr->y[next_local], 0, 3);
            PREFETCH(&next_chunk_ptr->width[next_local], 0, 3);
            PREFETCH(&next_chunk_ptr->height[next_local], 0, 3);
        }

        // Using direct access through macros
        float world_x = GET_ENTITY_X(entities, entity_idx);
        float world_y = GET_ENTITY_Y(entities, entity_idx);
        float width = (float)GET_ENTITY_WIDTH(entities, entity_idx);
        float height = (float)GET_ENTITY_HEIGHT(entities, entity_idx);
...
void update_dynamic_loading(Engine* engine) {
    ...
    // Update entity active states based on grid loading - process chunk by chunk for better cache coherency
    for (int c = 0; c < engine->entities.chunk_count; c++) {
        EntityChunk* chunk = engine->entities.chunks[c];

        // Prefetch the first entity data in the chunk
        if (chunk->count > 0) {
            PREFETCH(&chunk->x[0], 0, 3);
            PREFETCH(&chunk->y[0], 0, 3);
        }

        for (int i = 0; i < chunk->count; i++) {
            // Prefetch next entity data if available
            if (i + 1 < chunk->count) {
                PREFETCH(&chunk->x[i + 1], 0, 3);
                PREFETCH(&chunk->y[i + 1], 0, 3);
            }
...
void engine_update(Engine* engine) {
    ...
    // Rebuild spatial grid with active entities - process chunk by chunk for cache efficiency
    for (int c = 0; c < engine->entities.chunk_count; c++) {
        EntityChunk* chunk = engine->entities.chunks[c];

        // Prefetch first entity data in the chunk
        if (chunk->count > 0) {
            PREFETCH(&chunk->active[0], 0, 3);
            PREFETCH(&chunk->x[0], 0, 3);
            PREFETCH(&chunk->y[0], 0, 3);
        }
...
void engine_render(Engine* engine) {
    ...
    for (int i = 0; i < visible_count; i++) {
        ...
        // Use macros for direct access
        int texture_id = GET_ENTITY_TEXTURE_ID(&engine->entities, entity_idx);
        int layer = GET_ENTITY_LAYER(&engine->entities, entity_idx);
...
int engine_add_entity(Engine* engine, float x, float y, int width, int height,
    ...
    // Get chunk and local indices - using faster bitwise operations
    int chunk_idx = ENTITY_CHUNK_IDX(entity_idx);
    int local_idx = ENTITY_LOCAL_IDX(entity_idx);
...
void engine_set_entity_active(Engine* engine, int entity_idx, bool active) {
    ...
    // Use macros for direct access
    GET_ENTITY_ACTIVE(&engine->entities, entity_idx) = active;
}
