...
// Modified EntityChunk constructor to include type linking
...
Engine* engine_create(int window_width, int window_height, int world_width, int world_height, int cell_size) {
    ...
    engine->entity_type_capacity = 8;  // 256K initial capacity
...
// Efficient grid rebuilding
void clear_and_rebuild_grid(Engine* engine) {
    // Clear all cell counts (but keep memory allocated)
    for (int y = 0; y < engine->grid.height; y++) {
        memset(engine->grid.cell_counts[y], 0, engine->grid.width * sizeof(int));
    }

    // Process chunks in order for better cache locality
    for (int chunk_idx = 0; chunk_idx < engine->entities.chunk_count; chunk_idx++) {
        EntityChunk* chunk = engine->entities.chunks[chunk_idx];
        ...
        // Process in batch for better cache locality
        for (int local_idx = 0; local_idx < chunk->count; local_idx++) {
            if (chunk->active[local_idx]) {
...
void engine_update(Engine* engine) {
    ...
    // Update all entity types in parallel
    engine_update_entity_types(engine, delta_time);

    // Clear and rebuild spatial grid in one pass for better cache utilization
    clear_and_rebuild_grid(engine);
}
...
#pragma omp parallel for reduction(+:total_active_entities) schedule(dynamic)
    for (int type_idx = 0; type_idx < engine->entity_type_count; type_idx++) {
        ...
        while (chunk_idx != -1) {
            ...
            for (int j = 0; j < chunk->count; j++) {
                if (chunk->active[j]) active_count++;
            }

            if (active_count > 0) {
                // Call the update function - which handles all entities in the chunk
                config->update_func(chunk, chunk->count, delta_time);
                type_active_count += active_count;
            }

            // Move to next chunk of the same type using the linked list
            chunk_idx = chunk->next_chunk_of_type;
        }
...
void engine_set_entity_position(Engine* engine, int entity_idx, float x, float y) {
    ...
    float old_x = chunk->x[local_idx];
    float old_y = chunk->y[local_idx];
    ...
    // Update spatial grid if entity is active
    if (chunk->active[local_idx]) {
        // Calculate old grid cell
        int old_grid_x = (int)(old_x / engine->grid.cell_size);
        int old_grid_y = (int)(old_y / engine->grid.cell_size);

        // Calculate new grid cell
        int new_grid_x = (int)(x / engine->grid.cell_size);
        int new_grid_y = (int)(y / engine->grid.cell_size);

        // Only update grid if cell changed
        if (old_grid_x != new_grid_x || old_grid_y != new_grid_y) {
            // Clamp to grid bounds (old position)
            old_grid_x = std::max(0, std::min(old_grid_x, engine->grid.width - 1));
            old_grid_y = std::max(0, std::min(old_grid_y, engine->grid.height - 1));

            // Clamp to grid bounds (new position)
            new_grid_x = std::max(0, std::min(new_grid_x, engine->grid.width - 1));
            new_grid_y = std::max(0, std::min(new_grid_y, engine->grid.height - 1));

            // Remove from old cell
            int* cell = engine->grid.cells[old_grid_y][old_grid_x];
            int count = engine->grid.cell_counts[old_grid_y][old_grid_x];

            for (int i = 0; i < count; i++) {
                if (cell[i] == entity_idx) {
                    // Replace with last entity in cell and decrement count
                    cell[i] = cell[count - 1];
                    engine->grid.cell_counts[old_grid_y][old_grid_x]--;
                    break;
                }
            }
    ...
    // Update all child entities recursively if needed
