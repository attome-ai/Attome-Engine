...
EntityChunk::EntityChunk(int chunk_type_id, int chunk_capacity, size_t extra_data_size) {
    ...
    active = static_cast<bool*>(SDL_aligned_alloc(CACHE_LINE_SIZE, bool_size));
...
// Update the engine_create function to initialize next_type_id
...
void engine_update(Engine* engine) {
    ...
    {
        PROFILE_SCOPE("update_entity_transforms");
...
// Updated to include z_index parameter with default value
int engine_add_entity(Engine* engine, float x, float y, int width, int height,
    int texture_id, int layer, int z_index) {
    PROFILE_FUNCTION();
...
int engine_add_child_entity(Engine* engine, int parent_id, float local_x, float local_y,
    ...
    int entity_id = engine_add_entity(engine, parent_x + local_x, parent_y + local_y,
        width, height, texture_id, layer, z_index);  // Pass the z_index parameter
