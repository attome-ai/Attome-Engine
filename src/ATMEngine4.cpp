...
#define HASH_PRIME_1 73856093
#define HASH_PRIME_2 19349663
...
// Get cell index from world coordinates for spatial grid
...
void init_spatial_hash(SpatialHash* hash, float cell_size, int bucket_count) {
    hash->cell_size = cell_size;
    hash->bucket_count = bucket_count;
    hash->prime = 31; // A prime number for hashing

    hash->buckets = (int**)SDL_aligned_alloc(bucket_count * sizeof(int*), CACHE_LINE_SIZE);
    hash->bucket_counts = (int*)SDL_aligned_alloc(bucket_count * sizeof(int), CACHE_LINE_SIZE);
    hash->bucket_capacities = (int*)SDL_aligned_alloc(bucket_count * sizeof(int), CACHE_LINE_SIZE);

    for (int i = 0; i < bucket_count; i++) {
        hash->bucket_counts[i] = 0;
        hash->bucket_capacities[i] = 32; // Initial capacity
        hash->buckets[i] = (int*)SDL_aligned_alloc(hash->bucket_capacities[i] * sizeof(int), CACHE_LINE_SIZE);
    }
}
...
void free_spatial_hash(SpatialHash* hash) {
    for (int i = 0; i < hash->bucket_count; i++) {
        SDL_aligned_free(hash->buckets[i]);
    }
    SDL_aligned_free(hash->buckets);
    SDL_aligned_free(hash->bucket_counts);
    SDL_aligned_free(hash->bucket_capacities);
}

// Clear all buckets in the spatial hash
void clear_spatial_hash(SpatialHash* hash) {
    for (int i = 0; i < hash->bucket_count; i++) {
        hash->bucket_counts[i] = 0;
    }
}
...
int hash_position(SpatialHash* hash, float x, float y) {
    // Convert to grid coordinates
    int grid_x = (int)(x / hash->cell_size);
    int grid_y = (int)(y / hash->cell_size);

    // Cantor pairing function, then modulo with bucket count
    unsigned int h = ((grid_x + grid_y) * (grid_x + grid_y + 1) / 2) + grid_y;
    return h % hash->bucket_count;
}

// Add entity to spatial hash
void spatial_hash_add(SpatialHash* hash, int entity_idx, float x, float y) {
    int bucket_idx = hash_position(hash, x, y);

    // Ensure capacity
    if (hash->bucket_counts[bucket_idx] >= hash->bucket_capacities[bucket_idx]) {
        int new_capacity = hash->bucket_capacities[bucket_idx] * 2;
        int* new_bucket = (int*)SDL_aligned_alloc(new_capacity * sizeof(int), CACHE_LINE_SIZE);

        // Copy existing data
        memcpy(new_bucket, hash->buckets[bucket_idx],
            hash->bucket_counts[bucket_idx] * sizeof(int));

        // Free old bucket and update
        SDL_aligned_free(hash->buckets[bucket_idx]);
        hash->buckets[bucket_idx] = new_bucket;
        hash->bucket_capacities[bucket_idx] = new_capacity;
    }

    // Add entity to bucket
    hash->buckets[bucket_idx][hash->bucket_counts[bucket_idx]++] = entity_idx;
}

// Calculate hash indices for a query rectangle
void calculate_hash_indices(SpatialHash* hash, SDL_FRect query_rect, int* indices, int* count, int max_indices) {
    // Calculate grid coordinates for corners of the query rect
    int min_x = (int)(query_rect.x / hash->cell_size);
    int min_y = (int)(query_rect.y / hash->cell_size);
    int max_x = (int)((query_rect.x + query_rect.w) / hash->cell_size);
    int max_y = (int)((query_rect.y + query_rect.h) / hash->cell_size);

    *count = 0;

    // Hash each cell in the query area
    for (int y = min_y; y <= max_y; y++) {
        for (int x = min_x; x <= max_x; x++) {
            // Use Cantor pairing function
            unsigned int h = ((x + y) * (x + y + 1) / 2) + y;
            int bucket_idx = h % hash->bucket_count;

            // Check if this bucket is already in our list
            bool found = false;
            for (int i = 0; i < *count; i++) {
                if (indices[i] == bucket_idx) {
                    found = true;
                    break;
                }
            }

            if (!found && *count < max_indices) {
                indices[(*count)++] = bucket_idx;
            }
        }
    }
}

// Query hash for entities in a region
void spatial_hash_query(SpatialHash* hash, SDL_FRect query_rect, int* result_indices,
    int* result_count, int max_results, EntityManager* entities) {
    // Get hash buckets that overlap with the query rect
    int indices[256]; // Maximum number of hash buckets to check
    int count = 0;

    calculate_hash_indices(hash, query_rect, indices, &count, 256);

    *result_count = 0;

    // Check each bucket
    for (int i = 0; i < count && *result_count < max_results; i++) {
        int bucket_idx = indices[i];

        // Check each entity in the bucket
        for (int j = 0; j < hash->bucket_counts[bucket_idx] && *result_count < max_results; j++) {
            int entity_idx = hash->buckets[bucket_idx][j];

            if (entities->active[entity_idx] && entities->visible[entity_idx]) {
                // Check if entity overlaps with query rect
                float ex = entities->x[entity_idx];
                float ey = entities->y[entity_idx];
                float er = entities->right[entity_idx];
                float eb = entities->bottom[entity_idx];
...
QuadtreeNode* create_quadtree_node(SDL_FRect bounds, int capacity) {
    QuadtreeNode* node = (QuadtreeNode*)SDL_malloc(sizeof(QuadtreeNode));
    node->bounds = bounds;
    node->capacity = capacity;
    node->count = 0;
    node->entities = (int*)SDL_aligned_alloc(capacity * sizeof(int), CACHE_LINE_SIZE);
    node->is_leaf = true;
...
// Free a quadtree node and all its children recursively
void free_quadtree_node(QuadtreeNode* node) {
    if (!node) return;

    // Free children recursively
    for (int i = 0; i < 4; i++) {
        if (node->children[i]) {
            free_quadtree_node(node->children[i]);
        }
    }

    // Free this node's data
    SDL_aligned_free(node->entities);
    SDL_free(node);
}

// Initialize a quadtree
void init_quadtree(Quadtree* quadtree, SDL_FRect bounds, int node_capacity, int max_depth) {
    quadtree->bounds = bounds;
    quadtree->node_capacity = node_capacity;
    quadtree->max_depth = max_depth;
    quadtree->root = create_quadtree_node(bounds, node_capacity);
    quadtree->needs_rebuild = false;
}

// Free all resources used by the quadtree
void free_quadtree(Quadtree* quadtree) {
    if (quadtree->root) {
        free_quadtree_node(quadtree->root);
        quadtree->root = NULL;
    }
}

// Clear the quadtree for reuse
void clear_quadtree(Quadtree* quadtree) {
    // Just rebuild from scratch - more efficient for our use case
    free_quadtree(quadtree);
    quadtree->root = create_quadtree_node(quadtree->bounds, quadtree->node_capacity);
    quadtree->needs_rebuild = false;
}
...
void split_node(QuadtreeNode* node) {
    ...
    node->children[0] = create_quadtree_node(SDL_FRect{ x, y, w, h }, node->capacity);
    node->children[1] = create_quadtree_node(SDL_FRect{ x + w, y, w, h }, node->capacity);
    node->children[2] = create_quadtree_node(SDL_FRect{ x, y + h, w, h }, node->capacity);
    node->children[3] = create_quadtree_node(SDL_FRect{ x + w, y + h, w, h }, node->capacity);

    node->is_leaf = false;
}

// Get the child index for a point
int get_child_index(QuadtreeNode* node, float x, float y) {
    float mid_x = node->bounds.x + node->bounds.w / 2;
    float mid_y = node->bounds.y + node->bounds.h / 2;
...
bool insert_entity_recursive(QuadtreeNode* node, int entity_idx, float x, float y,
    EntityManager* entities, int depth, int max_depth) {
    // If point is not in bounds, don't add it
    if (x < node->bounds.x || x > node->bounds.x + node->bounds.w ||
        y < node->bounds.y || y > node->bounds.y + node->bounds.h) {
        return false;
    }

    // If node is a leaf and has space, insert directly
    if (node->is_leaf && node->count < node->capacity) {
        node->entities[node->count++] = entity_idx;
        return true;
    }

    // If we've reached max depth, force insert here
    if (depth >= max_depth) {
        // Just add it to this node, possibly exceeding capacity
        int* new_entities = (int*)SDL_aligned_alloc((node->count + 1) * sizeof(int), CACHE_LINE_SIZE);
        memcpy(new_entities, node->entities, node->count * sizeof(int));
        new_entities[node->count] = entity_idx;
    ...
    if (node->is_leaf) {
        ...
        for (int i = 0; i < node->count; i++) {
            int idx = node->entities[i];
            float ex = entities->x[idx];
            float ey = entities->y[idx];

            int child_idx = get_child_index(node, ex, ey);
            insert_entity_recursive(node->children[child_idx], idx, ex, ey,
                entities, depth + 1, max_depth);
        }

        // Clear this node's entities since they've been redistributed
        node->count = 0;
    }

    // Insert into appropriate child
    int child_idx = get_child_index(node, x, y);
    return insert_entity_recursive(node->children[child_idx], entity_idx, x, y,
        entities, depth + 1, max_depth);
}

// Insert an entity into the quadtree
void quadtree_insert(Quadtree* quadtree, int entity_idx, float x, float y, EntityManager* entities) {
    insert_entity_recursive(quadtree->root, entity_idx, x, y, entities, 0, quadtree->max_depth);
}

// Rebuild the entire quadtree
void rebuild_quadtree(Quadtree* quadtree, EntityManager* entities) {
    clear_quadtree(quadtree);

    // Reinsert all active entities
    for (int i = 0; i < entities->count; i++) {
        if (entities->active[i]) {
            quadtree_insert(quadtree, i, entities->x[i], entities->y[i], entities);
        }
    }
}

// Check if a rectangle intersects with a node's boundary
bool intersects(SDL_FRect a, SDL_FRect b) {
    return !(a.x + a.w <= b.x || a.x >= b.x + b.w ||
        a.y + a.h <= b.y || a.y >= b.y + b.h);
}

// Query entities that overlap with the given rectangle recursively
void query_node(QuadtreeNode* node, SDL_FRect query_rect, int* result_indices,
    int* result_count, int max_results, EntityManager* entities) {
    if (!node || !intersects(node->bounds, query_rect)) {
        return;
    }

    // Check entities in this node
    for (int i = 0; i < node->count && *result_count < max_results; i++) {
        int entity_idx = node->entities[i];
    ...
    if (!node->is_leaf) {
        for (int i = 0; i < 4 && *result_count < max_results; i++) {
            query_node(node->children[i], query_rect, result_indices,
                result_count, max_results, entities);
        }
    }
}

// Query entities in the quadtree that overlap with the given rectangle
void quadtree_query(Quadtree* quadtree, SDL_FRect query_rect, int* result_indices,
    int* result_count, int max_results, EntityManager* entities) {
    *result_count = 0;
    query_node(quadtree->root, query_rect, result_indices, result_count, max_results, entities);
}

/*
 * SIMD-Based Frustum Culling
 */
...
int simd_frustum_cull(EntityManager* entities, SDL_FRect view_rect, int* cell_entities,
    ...
    for (int i = a0; i < cell_entity_count; i += 4) {
        ...
        for (int j = 0; j < batch_size; j++) {
            if (!(mask & (1 << j)) && entities->active[indices_batch[j]] && entities->visible[indices_batch[j]]) {
    ...
    for (int i = 0; i < cell_entity_count; i += 4) {
        ...
        // Add visible entities to results
        for (int j = 0; j < batch_size; j++) {
            if (!mask[j] && entities->active[indices_batch[j]] && entities->visible[indices_batch[j]]) {
                result_indices[result_count++] = indices_batch[j];
            }
        }
    }
...
// Query entities in a region using SIMD-based frustum culling for grid
...
// Generic query function that routes to the appropriate spatial method
void spatial_query(Engine* engine, SDL_FRect query_rect, int* result_indices,
    int* result_count, int max_results) {
    switch (engine->spatial_method) {
    case SPATIAL_PARTITIONING_GRID:
        spatial_grid_query(&engine->grid, query_rect, result_indices,
            result_count, max_results, &engine->entities);
        break;

    case SPATIAL_PARTITIONING_HASH:
        spatial_hash_query(&engine->hash, query_rect, result_indices,
            result_count, max_results, &engine->entities);
        break;

    case SPATIAL_PARTITIONING_QUADTREE:
        quadtree_query(&engine->quadtree, query_rect, result_indices,
            result_count, max_results, &engine->entities);
        break;
    }
}
...
void init_entity_manager(EntityManager* manager, int initial_capacity) {
    ...
    manager->visible = (bool*)SDL_aligned_alloc(initial_capacity * sizeof(bool), CACHE_LINE_SIZE);
...
void free_entity_manager(EntityManager* manager) {
    ...
    SDL_aligned_free(manager->visible);
...
/*
 * Camera and View Implementation
 */
...
Engine* engine_create(int window_width, int window_height, int world_width, int world_height) {
    ...
    // Init spatial grid (default method)
    init_spatial_grid(&engine->grid, world_width, world_height, 128.0f);

    // Init spatial hash
    init_spatial_hash(&engine->hash, 128.0f, 16384); // 16K buckets for large worlds

    // Init quadtree
    init_quadtree(&engine->quadtree, engine->world_bounds, 16, 8);

    // Set default spatial method
    engine->spatial_method = SPATIAL_PARTITIONING_GRID;
...
void engine_destroy(Engine* engine) {
    ...
    // Free spatial hash
    free_spatial_hash(&engine->hash);

    // Free quadtree
    free_quadtree(&engine->quadtree);
...
void engine_update(Engine* engine) {
    ...
    // Process spatial partitioning updates based on current method
    switch (engine->spatial_method) {
    case SPATIAL_PARTITIONING_GRID:
        // Clear spatial grid
    ...
    case SPATIAL_PARTITIONING_HASH:
        // Clear spatial hash
        clear_spatial_hash(&engine->hash);

        // Rebuild spatial hash with active entities
        for (int i = 0; i < engine->entities.count; i++) {
            if (engine->entities.active[i]) {
                spatial_hash_add(&engine->hash, i, engine->entities.x[i], engine->entities.y[i]);
            }
        }
        break;

    case SPATIAL_PARTITIONING_QUADTREE:
        // Check if quadtree needs rebuilding
        if (engine->quadtree.needs_rebuild) {
            rebuild_quadtree(&engine->quadtree, &engine->entities);
            engine->quadtree.needs_rebuild = false;
        }
        break;
    }
}
...
void engine_render(Engine* engine) {
    ...
    // Query visible entities from current spatial partitioning structure
    spatial_query(engine, visible_rect, visible_indices, &visible_count,
...
int engine_add_entity(Engine* engine, float x, float y, int width, int height,
    ...
    if (em->count >= em->capacity) {
        ...
        bool* new_visible = (bool*)SDL_aligned_alloc(new_capacity * sizeof(bool), CACHE_LINE_SIZE);
        ...
        memcpy(new_visible, em->visible, em->count * sizeof(bool));
    ...
    em->visible[entity_idx] = true;

    // Add to current spatial partitioning structure
    switch (engine->spatial_method) {
    case SPATIAL_PARTITIONING_GRID:
    ...
    case SPATIAL_PARTITIONING_HASH:
        spatial_hash_add(&engine->hash, entity_idx, x, y);
        break;

    case SPATIAL_PARTITIONING_QUADTREE:
        quadtree_insert(&engine->quadtree, entity_idx, x, y, &engine->entities);
        break;
    }
...
void engine_set_entity_position(Engine* engine, int entity_idx, float x, float y) {
    if (entity_idx >= 0 && entity_idx < engine->entities.count) {
        engine->entities.x[entity_idx] = x;
        engine->entities.y[entity_idx] = y;

        // Update precomputed values
        engine->entities.right[entity_idx] = x + engine->entities.width[entity_idx];
        engine->entities.bottom[entity_idx] = y + engine->entities.height[entity_idx];

        // Mark quadtree for rebuild if using it
        if (engine->spatial_method == SPATIAL_PARTITIONING_QUADTREE) {
            engine->quadtree.needs_rebuild = true;
        }
    }
}
...
void engine_set_entity_visible(Engine* engine, int entity_idx, bool visible) {
    if (entity_idx >= 0 && entity_idx < engine->entities.count) {
        engine->entities.visible[entity_idx] = visible;
    }
}

void engine_set_spatial_method(Engine* engine, SpatialPartitioningMethod method) {
    // If already using this method, do nothing
    if (engine->spatial_method == method) {
        return;
    }

    engine->spatial_method = method;

    // If switching to quadtree, we need to build it
    if (method == SPATIAL_PARTITIONING_QUADTREE) {
        engine->quadtree.needs_rebuild = true;
    }
}
