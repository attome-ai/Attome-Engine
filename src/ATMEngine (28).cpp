...
void radix_sort_entities(int* indices, int count, EntityManager* entities) {
    ...
    int* temp_indices = static_cast<int*>(SDL_aligned_alloc(CACHE_LINE_SIZE, count * sizeof(int)));
    ...
    // First sort by z_index (changed from texture_id)
    int count_z_index[RADIX_SIZE] = { 0 };

    // Count frequencies for z_index
    for (int i = 0; i < count; i++) {
        ...
        EntityChunk* chunk = entities->chunks[chunk_idx];
        int z_index = chunk->z_index[local_idx] & RADIX_MASK;
        count_z_index[z_index]++;
    }

    // Compute prefix sum
    int total = 0;
    for (int i = 0; i < RADIX_SIZE; i++) {
        int temp = count_z_index[i];
        count_z_index[i] = total;
        total += temp;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        EntityChunk* chunk = entities->chunks[chunk_idx];
        int z_index = chunk->z_index[local_idx] & RADIX_MASK;
        temp_indices[count_z_index[z_index]++] = entity_idx;
    }

    // Copy back to original array
    memcpy(indices, temp_indices, count * sizeof(int));

    // Now sort by texture_id within each z_index group
    // First identify z_index groups
    int z_index_start[RADIX_SIZE];
    int z_index_count[RADIX_SIZE];
    ...
    // Sort each z_index group by texture_id
    for (int z = 0; z < RADIX_SIZE; z++) {
        int start = z_index_start[z];
        int size = z_index_count[z];
        ...
        for (int i = 0; i < RADIX_SIZE; i++) {
            ...
            count_texture[i] = group_total;
            group_total += temp;
        }

        // Sort this group by texture_id
        for (int i = 0; i < size; i++) {
            int entity_idx = indices[start + i];
            ...
            EntityChunk* chunk = entities->chunks[chunk_idx];
            int texture_id = chunk->texture_id[local_idx] & RADIX_MASK;
            temp_indices[start + count_texture[texture_id]++] = entity_idx;
        }

        // Copy back to original array just for this z_index group
