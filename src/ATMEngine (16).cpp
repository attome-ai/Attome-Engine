...
QuadTreeNode* quadtree_node_create(SDL_FRect boundary, int max_entities, int depth, int max_depth) {
    QuadTreeNode* node = static_cast<QuadTreeNode*>(malloc(sizeof(QuadTreeNode)));

    node->boundary = boundary;
    node->entity_capacity = max_entities;
    node->entity_count = 0;
    node->is_divided = false;
    node->entities = static_cast<int*>(malloc(max_entities * sizeof(int)));
...
// Destroy a QuadTree node (recursive)
void quadtree_node_destroy(QuadTreeNode* node) {
    if (!node) return;

    // Free child nodes recursively
    if (node->is_divided) {
        quadtree_node_destroy(node->northwest);
        quadtree_node_destroy(node->northeast);
        quadtree_node_destroy(node->southwest);
        quadtree_node_destroy(node->southeast);
    }
...
bool quadtree_node_intersects(SDL_FRect a, SDL_FRect b) {
    return !(b.x > a.x + a.w ||
        b.x + b.w < a.x ||
        b.y > a.y + a.h ||
        b.y + b.h < a.y);
...
void quadtree_node_subdivide(QuadTreeNode* node) {
    ...
    SDL_FRect nw = { x, y, half_w, half_h };
    SDL_FRect ne = { x + half_w, y, half_w, half_h };
    SDL_FRect sw = { x, y + half_h, half_w, half_h };
    SDL_FRect se = { x + half_w, y + half_h, half_w, half_h };

    node->northwest = quadtree_node_create(nw, node->entity_capacity, 0, 0);
    node->northeast = quadtree_node_create(ne, node->entity_capacity, 0, 0);
    node->southwest = quadtree_node_create(sw, node->entity_capacity, 0, 0);
    node->southeast = quadtree_node_create(se, node->entity_capacity, 0, 0);

    node->is_divided = true;
}

// Insert an entity into the QuadTree
bool quadtree_node_insert(QuadTreeNode* node, int entity_idx, SDL_FRect entity_rect, int depth, int max_depth, int max_entities) {
    // If the entity doesn't intersect with this node, don't insert
    if (!quadtree_node_intersects(node->boundary, entity_rect)) {
        return false;
    }

    // If there's space in this node and it's a leaf or we're at max depth, insert here
    if (node->entity_count < node->entity_capacity || depth >= max_depth) {
        if (node->entity_count >= node->entity_capacity) {
            // Expand capacity if needed
            int new_capacity = node->entity_capacity * 2;
            node->entities = static_cast<int*>(realloc(node->entities, new_capacity * sizeof(int)));
            node->entity_capacity = new_capacity;
        }

        node->entities[node->entity_count++] = entity_idx;
        return true;
    }

    // If we need to subdivide, do so
    if (!node->is_divided) {
    ...
    bool inserted = false;
    inserted |= quadtree_node_insert(node->northwest, entity_idx, entity_rect, depth + 1, max_depth, max_entities);
    inserted |= quadtree_node_insert(node->northeast, entity_idx, entity_rect, depth + 1, max_depth, max_entities);
    inserted |= quadtree_node_insert(node->southwest, entity_idx, entity_rect, depth + 1, max_depth, max_entities);
    inserted |= quadtree_node_insert(node->southeast, entity_idx, entity_rect, depth + 1, max_depth, max_entities);

    // If we couldn't insert into any children (rare), insert here
    if (!inserted) {
        if (node->entity_count >= node->entity_capacity) {
            // Expand capacity if needed
            int new_capacity = node->entity_capacity * 2;
...
void quadtree_node_query(QuadTreeNode* node, SDL_FRect query_rect, int* result_indices, int* result_count, int max_results, EntityManager* entities) {
    // If this node doesn't intersect with the query, return
    if (!quadtree_node_intersects(node->boundary, query_rect)) {
        return;
    }

    // Add entities from this node that are within the query rect
    for (int i = 0; i < node->entity_count && *result_count < max_results; i++) {
        ...
        SDL_FRect entity_rect = { ex, ey, er - ex, eb - ey };

        // Check if entity is in query rect
        if (quadtree_node_intersects(entity_rect, query_rect)) {
            result_indices[(*result_count)++] = entity_idx;
        }
    }

    // If this node is divided, check its children
    if (node->is_divided) {
        quadtree_node_query(node->northwest, query_rect, result_indices, result_count, max_results, entities);
        quadtree_node_query(node->northeast, query_rect, result_indices, result_count, max_results, entities);
        quadtree_node_query(node->southwest, query_rect, result_indices, result_count, max_results, entities);
        quadtree_node_query(node->southeast, query_rect, result_indices, result_count, max_results, entities);
    }
}

// Initialize QuadTree
void init_quadtree(QuadTree* quadtree, float world_width, float world_height, int max_entities, int max_depth) {
    quadtree->max_entities = max_entities;
    quadtree->max_depth = max_depth;
    quadtree->world_width = world_width;
    quadtree->world_height = world_height;

    SDL_FRect boundary = { 0, 0, world_width, world_height };
    quadtree->root = quadtree_node_create(boundary, max_entities, 0, max_depth);
}

// Clear QuadTree (remove all entities, but keep structure)
void clear_quadtree(QuadTree* quadtree) {
    // Destroy old tree
    quadtree_node_destroy(quadtree->root);

    // Create new root node
    SDL_FRect boundary = { 0, 0, quadtree->world_width, quadtree->world_height };
    quadtree->root = quadtree_node_create(boundary, quadtree->max_entities, 0, quadtree->max_depth);
}
...
void quadtree_add(QuadTree* quadtree, int entity_idx, float x, float y, float width, float height) {
    SDL_FRect entity_rect = { x, y, width, height };
    quadtree_node_insert(quadtree->root, entity_idx, entity_rect, 0, quadtree->max_depth, quadtree->max_entities);
}

// Query entities in a region
void quadtree_query(QuadTree* quadtree, SDL_FRect query_rect, int* result_indices, int* result_count, int max_results, EntityManager* entities) {
    *result_count = 0;
    quadtree_node_query(quadtree->root, query_rect, result_indices, result_count, max_results, entities);
}
...
Engine* engine_create(int window_width, int window_height, int world_width, int world_height, int cell_size) {
    ...
    // Initialize quadtree (replacing spatial grid)
    init_quadtree(&engine->quadtree, world_width, world_height, 16, 8);
...
// Modified engine_update to include entity type updates and use quadtree instead of grid
void engine_update(Engine* engine) {
    ...
    clear_quadtree(&engine->quadtree);

    // Rebuild quadtree with visible entities (not just active)
    for (int chunk_idx = 0; chunk_idx < engine->entities.chunk_count; chunk_idx++) {
        EntityChunk* chunk = engine->entities.chunks[chunk_idx];
        ...
        for (int local_idx = 0; local_idx < chunk->count; local_idx++) {
            if (chunk->visible[local_idx]) {  // Changed from active to visible
                ...
                float x = chunk->x[local_idx];
                float y = chunk->y[local_idx];
                float width = chunk->width[local_idx];
                float height = chunk->height[local_idx];

                quadtree_add(&engine->quadtree, entity_idx, x, y, width, height);
            }
        }
    }
}
...
void engine_render(Engine* engine) {
    ...
    // Query visible entities from quadtree instead of spatial grid
    quadtree_query(&engine->quadtree, visible_rect, visible_indices, &visible_count,
...
int engine_get_visible_entities_count(Engine* engine) {
    ...
    // Use quadtree query instead of spatial grid
    quadtree_query(&engine->quadtree, visible_rect, visible_indices, &visible_count,
        engine->entities.total_count, &engine->entities);
...
void engine_update_entity_transforms(Engine* engine) {
    // First pass: Update all root entities (they use local position as world position)
    for (int chunk_idx = 0; chunk_idx < engine->entities.chunk_count; chunk_idx++) {
        EntityChunk* chunk = engine->entities.chunks[chunk_idx];
    ...
    // Use a work queue for breadth-first traversal (better for cache locality)
    const int MAX_QUEUE_SIZE = 100000; // Adjust as needed
    int* queue = static_cast<int*>(SDL_aligned_alloc(MAX_QUEUE_SIZE * sizeof(int), CACHE_LINE_SIZE));
    int queue_head = 0;
    int queue_tail = 0;

    // Enqueue all root entities with children
    for (int chunk_idx = 0; chunk_idx < engine->entities.chunk_count; chunk_idx++) {
        EntityChunk* chunk = engine->entities.chunks[chunk_idx];
        ...
        for (int local_idx = 0; local_idx < chunk->count; local_idx++) {
            ...
            if (chunk->parent_id[local_idx] == -1 && chunk->first_child_id[local_idx] != -1) {
    ...
    // Process entities in breadth-first order
...
void engine_update_entity_types(Engine* engine, float delta_time) {
    ...
    for (int type_idx = 0; type_idx < engine->entity_type_count; type_idx++) {
        ...
        while (chunk_idx != -1) {
            ...
            if (active_count > 0) {
                // Get original positions for any that move (use stack for small chunks)
                float* original_x = active_count > 64 ?
                    new float[active_count] :
                    (float*)alloca(active_count * sizeof(float));

                float* original_y = active_count > 64 ?
                    new float[active_count] :
                    (float*)alloca(active_count * sizeof(float));

                // Collect original positions of active entities only
                int active_idx = 0;
                int* active_to_chunk_map = active_count > 64 ?
                    new int[active_count] :
                    (int*)alloca(active_count * sizeof(int));

                for (int j = 0; j < chunk->count; j++) {
                    if (chunk->active[j]) {  // Check active, not visible
                ...
                // Call update only if there are active entities
                if (active_count > 0) {
