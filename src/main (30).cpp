...
#define MIN_ENTITIES_PER_TYPE 100        // Minimum entities per type
...
void generic_entity_update(EntityChunk* chunk, int count, float delta_time) {
    ...
    int i = 0;
    for (; i + 3 < count; i += 4) {
        // Quick check if all 4 entities are inactive, skip block if so
        if (!chunk->active[i] && !chunk->active[i + 1] && !chunk->active[i + 2] && !chunk->active[i + 3]) {
            continue;
        }
    ...
    // Process remaining entities (count % 4) using scalar code
    for (; i < count; i++) {
