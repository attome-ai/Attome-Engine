...
#define QUADTREE_MAX_DEPTH 8  // Maximum depth for quadtree
#define QUADTREE_MAX_ENTITIES 16  // Maximum entities per quadtree node
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    // Initialize game engine with quadtree parameters
    state->engine = engine_create(WINDOW_WIDTH, WINDOW_HEIGHT, WORLD_WIDTH, WORLD_HEIGHT, QUADTREE_MAX_ENTITIES);
    if (!state->engine) {
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    if (!state->game_over) {
        ...
        // Use the quadtree for efficient querying
        const int MAX_VISIBLE = 10000;
        int* visible_entities = (int*)SDL_aligned_alloc(MAX_VISIBLE * sizeof(int), CACHE_LINE_SIZE);
        int visible_count = 0;

        // Use quadtree_query instead of spatial_grid_query
        quadtree_query(&engine->quadtree, visible_rect, visible_entities, &visible_count, MAX_VISIBLE, &engine->entities);
