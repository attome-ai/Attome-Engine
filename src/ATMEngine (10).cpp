...
// Modified EntityChunk constructor
...
// Modified EntityChunk destructor
...
Engine* engine_create(int window_width, int window_height, int world_width, int world_height, int cell_size) {
    ...
    engine->entity_type_capacity = 8;  // Initial capacity for entity types
...
void engine_set_camera_position(Engine* engine, float x, float y) {
    engine->camera.x = SDL_floorf(x);
    engine->camera.y = SDL_floor(y);
}
...
void engine_register_entity_type(Engine* engine, int type_id, EntityTypeUpdateFunc update_func, size_t extra_data_size) {
    ...
    if (engine->entity_type_count >= engine->entity_type_capacity) {
        engine->entity_type_capacity *= 2;
        engine->entity_types = static_cast<EntityTypeConfig*>(realloc(
            engine->entity_types,
            engine->entity_type_capacity * sizeof(EntityTypeConfig)
        ));
    }

    // Add new type
    EntityTypeConfig* config = &engine->entity_types[engine->entity_type_count++];
...
int find_or_create_chunk_for_type(Engine* engine, int type_id) {
    ...
    for (int i = 0; i < engine->entities.chunk_count; i++) {
        EntityChunk* chunk = engine->entities.chunks[i];
        if (chunk->type_id == type_id && chunk->count < chunk->capacity) {
            return i;
        }
    }

    // Find the entity type config to get extra data size
    size_t extra_data_size = 0;
    for (int i = 0; i < engine->entity_type_count; i++) {
        if (engine->entity_types[i].type_id == type_id) {
    ...
    int chunk_idx = engine->entities.chunk_count++;
    engine->entities.chunks[chunk_idx] = new EntityChunk(type_id, ENTITY_CHUNK_SIZE, extra_data_size);
...
int engine_add_entity_with_type(Engine* engine, int type_id, float x, float y,
    ...
    // Get entity index from EntityManager to properly update tracking
    int entity_idx = engine->entities.addEntity();

    // Get the actual chunk index and local index where the entity was created
    int actual_chunk_idx, local_idx;
    engine->entities.getChunkIndices(entity_idx, &actual_chunk_idx, &local_idx);

    // If entity wasn't created in our desired chunk, handle this special case
    if (actual_chunk_idx != chunk_idx) {
        // Make the entity inactive in its original chunk
        engine->entities.chunks[actual_chunk_idx]->active[local_idx] = false;

        // Use our typed chunk instead
        local_idx = chunk->count++;
        entity_idx = chunk_idx * ENTITY_CHUNK_SIZE + local_idx;
    ...
    // Set active status based on grid loading
    int grid_x = (int)(x / engine->grid_cell_size);
    int grid_y = (int)(y / engine->grid_cell_size);
...
void engine_update_entity_types(Engine* engine, float delta_time) {
    ...
    for (int chunk_idx = 0; chunk_idx < engine->entities.chunk_count; chunk_idx++) {
        EntityChunk* chunk = engine->entities.chunks[chunk_idx];
        if (chunk->count == 0 || chunk->type_id == -1) continue;

        // Find matching type config
        for (int i = 0; i < engine->entity_type_count; i++) {
            if (engine->entity_types[i].type_id == chunk->type_id) {
                EntityTypeConfig* config = &engine->entity_types[i];

                // Call the update function if available
                if (config->update_func) {
                    // Make a backup of positions before update to detect truncation issues
                    std::vector<float> original_x(chunk->count);
                    std::vector<float> original_y(chunk->count);

                    for (int j = 0; j < chunk->count; j++) {
                        original_x[j] = chunk->x[j];
                        original_y[j] = chunk->y[j];
                    }

                    // Call the update function
                    config->update_func(chunk, chunk->count, delta_time);

                    // Now ensure positions remain as floats by explicitly updating derived values
                    for (int j = 0; j < chunk->count; j++) {
                        // Preserve fractional components - skip if position hasn't changed
                        if (chunk->x[j] != original_x[j] || chunk->y[j] != original_y[j]) {
                            // Make sure we're using the float values directly
                            float new_x = chunk->x[j];
                            float new_y = chunk->y[j];

                            // Update dependent values using these exact float values
                            chunk->right[j] = new_x + static_cast<float>(chunk->width[j]);
                            chunk->bottom[j] = new_y + static_cast<float>(chunk->height[j]);

                            // If this is a child entity, update local position
                            if (chunk->parent_id[j] != -1) {
                                int parent_id = chunk->parent_id[j];
                                int parent_chunk_idx, parent_local_idx;
                                engine->entities.getChunkIndices(parent_id, &parent_chunk_idx, &parent_local_idx);
                                EntityChunk* parent_chunk = engine->entities.chunks[parent_chunk_idx];

                                chunk->local_x[j] = new_x - parent_chunk->x[parent_local_idx];
                                chunk->local_y[j] = new_y - parent_chunk->y[parent_local_idx];
                            }
                            else {
                                // For root entities, local position equals world position
                                chunk->local_x[j] = new_x;
                                chunk->local_y[j] = new_y;
                            }
                        }
                    }
                }
