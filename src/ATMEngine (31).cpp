...
void engine_update_entity_types(Engine* engine, float delta_time) {
    ...
    // Use static arrays to avoid stack reallocation on each function call
    static float original_x_stack[ENTITY_CHUNK_SIZE];
    static float original_y_stack[ENTITY_CHUNK_SIZE];
    static int active_to_chunk_map_stack[ENTITY_CHUNK_SIZE];
    ...
    for (int type_idx = 0; type_idx < engine->entity_type_count; type_idx++) {
        ...
        while (chunk_idx != -1) {
            ...
            int active_count = chunk->count;
            if (active_count <= 0) {
                chunk_idx = chunk->next_chunk_of_type;
                continue;
            }

            // Store original positions in a single pass
            // This is more cache-friendly than separate loops
            for (int j = 0; j < active_count; j++) {
                original_x_stack[j] = chunk->x[j];
                original_y_stack[j] = chunk->y[j];
                active_to_chunk_map_stack[j] = j;
            }

            // Call the update function
            config->update_func(chunk, active_count, delta_time);

            // Process updates in batches for improved cache locality
            // Use a direct comparison rather than function calls where possible
            for (int j = 0; j < active_count; j++) {
                int entity_idx = active_to_chunk_map_stack[j];
                float new_x = chunk->x[entity_idx];
                float new_y = chunk->y[entity_idx];

                // Skip if position didn't change (early termination)
                if (new_x == original_x_stack[j] && new_y == original_y_stack[j]) {
                    continue;
                }

                // Update dependent values only when needed
                // Avoid the static_cast when possible by using floats directly
                chunk->right[entity_idx] = new_x + chunk->width[entity_idx];
                chunk->bottom[entity_idx] = new_y + chunk->height[entity_idx];

                // Optimize parent-child relationships by checking parent_id first
                ...
                if (parent_id != -1) {
                    ...
                    // Direct pointer access to parent chunk is more efficient
                    ...
                    // Calculate local coordinates directly
                    chunk->local_x[entity_idx] = new_x - parent_x;
                    chunk->local_y[entity_idx] = new_y - parent_y;
                }
                else {
                    // Simple assignment when no parent
            ...
            // Advance to next chunk of the same type
            chunk_idx = chunk->next_chunk_of_type;
        }
...
// Add a new function to set entity z_index
void engine_set_entity_z_index(Engine* engine, int entity_idx, int z_index) {
    ...
    EntityChunk* chunk = engine->entities.chunks[chunk_idx];
    chunk->z_index[local_idx] = z_index;
}
