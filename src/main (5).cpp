...
#define WORLD_WIDTH 260000
#define WORLD_HEIGHT 220000
#define GRID_CELL_SIZE 256  // Added grid cell size parameter
...
// Check if two entities are colliding - Updated for chunked entity architecture
bool check_collision(Engine* engine, int entity1, int entity2) {
    ...
    get_chunk_indices(entity1, &chunk_idx1, &local_idx1);
    get_chunk_indices(entity2, &chunk_idx2, &local_idx2);

    // Check if valid entities
    if (chunk_idx1 >= engine->entities.chunk_count || chunk_idx2 >= engine->entities.chunk_count ||
        local_idx1 >= engine->entities.chunks[chunk_idx1]->count ||
        local_idx2 >= engine->entities.chunks[chunk_idx2]->count) {
        return false;
    }

    float x1 = engine->entities.chunks[chunk_idx1]->x[local_idx1];
    float y1 = engine->entities.chunks[chunk_idx1]->y[local_idx1];
    float r1 = engine->entities.chunks[chunk_idx1]->right[local_idx1];
    float b1 = engine->entities.chunks[chunk_idx1]->bottom[local_idx1];

    float x2 = engine->entities.chunks[chunk_idx2]->x[local_idx2];
    float y2 = engine->entities.chunks[chunk_idx2]->y[local_idx2];
    float r2 = engine->entities.chunks[chunk_idx2]->right[local_idx2];
    float b2 = engine->entities.chunks[chunk_idx2]->bottom[local_idx2];
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    // Get player position using the chunked entity access
    int player_chunk_idx, player_local_idx;
    get_chunk_indices(state->player_entity, &player_chunk_idx, &player_local_idx);
    ...
    // Update obstacles
    for (int i = 0; i < NUM_OBSTACLES; i++) {
        // Get obstacle position using chunked entity access
        int obstacle_chunk_idx, obstacle_local_idx;
        get_chunk_indices(state->obstacle_entities[i], &obstacle_chunk_idx, &obstacle_local_idx);

        float obstacle_x = engine->entities.chunks[obstacle_chunk_idx]->x[obstacle_local_idx];
        float obstacle_y = engine->entities.chunks[obstacle_chunk_idx]->y[obstacle_local_idx];
